...
This runbook explains how to roll the structured withdrawal threshold telemetry from staging into any environment (production, testnet, etc.) so alert calibrations are continuously archived, visualized, and pushed to downstream runbooks (GitHub Issues, Slack, Jira). It also includes a verification CLI (`scripts/tools/verify_monitoring.py --environment <env>` with `WITHDRAWAL_HISTORY_DIR`, `LOKI_URL`, and `LOKI_BEARER_TOKEN` env vars) that checks the local history file and optionally runs a Loki query.
...
1. **GitHub Issue (optional but recommended)**
   - Repository variable: `WITHDRAWAL_CALIBRATION_ISSUE` → the issue number that should receive comments.
   - Permission: the built-in `GITHUB_TOKEN` already has `issues:write` inside the workflow context.
...
2. **Slack (optional)**
   ...
   - Production deployments should store the secret in the corresponding GitHub environment so staging/prod webhooks remain isolated.
...
3. **Jira (optional)**
   ...
   - Permissions: the API token must have comment permissions on the chosen issue.
    - Keep separate Jira issue IDs for staging vs. production and scope the secrets to the matching GitHub environment.
...
- [ ] GitHub Actions run completed with the “Store threshold history” step showing `max-history-entries=500`.
...
- [ ] Slack/Jira/GitHub issue comments appear after the next deploy (check workflow logs for “Publish threshold summary to runbooks” step).
...
- [ ] **Retention configured** – either the default `--max-history-entries 500` runs after every GitHub Actions deploy or a cron job executes `threshold_artifact_ingest.py --max-history-entries <N>` on the host so the JSONL file never grows without bound.
...
- [ ] **Runbook publishers configured** – confirm the production GitHub environment/workflow has the required secrets/variables (`WITHDRAWAL_CALIBRATION_SLACK_WEBHOOK`, `WITHDRAWAL_CALIBRATION_ISSUE`, `WITHDRAWAL_CALIBRATION_JIRA_ISSUE`, `JIRA_BASE_URL`, `JIRA_EMAIL`, `JIRA_API_TOKEN`) and dry-run the publisher locally with `python scripts/tools/threshold_artifact_publish.py --details <file> --markdown <file> --dry-run` to verify payloads.
- [ ] **Alert destinations confirmed** – GitHub issue, Slack webhook, and/or Jira ticket settings (secrets + variables) are present in the production workflow so every deploy continues pushing summaries to the right runbooks.
