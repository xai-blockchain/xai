...
class PeerManager:
    ...
    def ban_peer(self, peer_identifier: str):
        ...
        peers_to_disconnect = [pid for pid, peer_info in self.connected_peers.items() 
    ...
    def can_connect(self, ip_address: str) -> bool:
        ...
        if self.connections_by_ip[ip_lower] >= self.max_connections_per_ip:
            print(f"Connection from {ip_address} rejected: Exceeds max connections per IP ({self.max_connections_per_ip}).")
            return False
    ...
    def connect_peer(self, ip_address: str) -> str:
        ...
        if not self.can_connect(ip_address):
            raise ValueError(f"Cannot connect to peer from {ip_address} due to policy restrictions.")
