...
class SybilResistanceManager:
    def __init__(self, minimum_reputation_for_action: float = 50.0):
        if not isinstance(minimum_reputation_for_action, (int, float)) or not (0 <= minimum_reputation_for_action <= 100):
        ...
        print(f"SybilResistanceManager initialized. Minimum reputation for action: {self.minimum_reputation_for_action:.2f}.")
    ...
    def register_personhood(self, participant_address: str):
        ...
        if participant_address in self.proof_of_personhood_registry:
            raise ValueError(f"Participant {participant_address} already registered for personhood.")
    ...
    def update_reputation(self, participant_address: str, change: float):
        ...
        print(f"Reputation for {participant_address} updated from {current_reputation:.2f} to {new_reputation:.2f}.")
    ...
    def check_participant_for_action(self, participant_address: str) -> bool:
        ...
        if participant_address not in self.proof_of_personhood_registry:
            print(f"Participant {participant_address} not registered for proof-of-personhood. Action denied.")
            return False

        reputation = self.get_reputation(participant_address)
        if reputation >= self.minimum_reputation_for_action:
            print(f"Participant {participant_address} meets reputation requirement ({reputation:.2f} >= {self.minimum_reputation_for_action:.2f}). Action allowed.")
            return True
        else:
            print(f"Participant {participant_address} does not meet reputation requirement ({reputation:.2f} < {self.minimum_reputation_for_action:.2f}). Action denied.")
            return False
...
if __name__ == "__main__":
    ...
    print(f"Sybil user can perform action: {sybil_manager.check_participant_for_action(sybil_user)}")
