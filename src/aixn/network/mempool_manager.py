...
class MempoolManager:
    def __init__(self, max_transactions: int = 1000, eviction_policy: str = "fifo"):
        ...
        print(f"MempoolManager initialized. Max transactions: {self.max_transactions}, Eviction policy: {self.eviction_policy}.")
    ...
    def add_transaction(self, transaction_details: Any, fee: float = 0.0) -> str:
        ...
        if len(self.pending_transactions) >= self.max_transactions:
            ...
            if self.eviction_policy == "fifo":
                ...
                oldest_tx_id = min(self.pending_transactions, key=lambda k: self.pending_transactions[k]["timestamp"])
                ...
                if self.transaction_queue:
                    ...
                    print(f"Evicted lowest fee transaction: {evicted_tx_id} with fee {lowest_fee_tx[0]:.2f}")
                else:
                    # Should not happen if pending_transactions is not empty
                    raise RuntimeError("Mempool full but transaction_queue is empty for lowest_fee policy.")
        ...
        if self.eviction_policy == "lowest_fee":
            heapq.heappush(self.transaction_queue, (fee, current_time, tx_id)) # Use timestamp as tie-breaker
        
        print(f"Added transaction {tx_id} (Fee: {fee:.2f}). Mempool size: {len(self.pending_transactions)}")
        return tx_id
