...
class PartitionDetector:
    def __init__(self, node_id: str, known_peers: List[str]):
        ...
        self.peer_reachability: Dict[str, Set[str]] = {self.node_id: set()} # Initialize own reachability
        for peer in known_peers:
            self.peer_reachability[peer] = set() # Initialize other peers' reachability

        print(f"PartitionDetector initialized for node {self.node_id}. Known peers: {self.known_peers}.")
    ...
    def update_own_reachability(self, reachable_peers: List[str]):
        ...
        print(f"Node {self.node_id} updated its reachability: {self.peer_reachability[self.node_id]}")
    ...
    def report_peer_reachability(self, reporting_peer_id: str, reachable_peers: List[str]):
        ...
        self.peer_reachability[reporting_peer_id] = set(reachable_peers).intersection(self.known_peers.union({self.node_id}))
        print(f"Peer {reporting_peer_id} reported reachability: {self.peer_reachability[reporting_peer_id]}")
    ...
    def detect_partitions(self) -> bool:
        ...
        if len(visited) < len(all_nodes):
            ...
            print(f"!!! NETWORK PARTITION DETECTED !!! Node {self.node_id} can reach {len(visited)} nodes, "
