...
from src.aixn.network.fee_adjuster import FeeAdjuster
...
class PriorityFeeManager:
    ...
    def calculate_total_fee(self, priority_fee: float) -> float:
        ...
        if not isinstance(priority_fee, (int, float)) or priority_fee < self.min_priority_fee:
            raise ValueError(f"Priority fee ({priority_fee:.4f}) must be at least the minimum ({self.min_priority_fee:.4f}).")
        
        base_fee = self.fee_adjuster.get_suggested_fee()
        total_fee = base_fee + priority_fee
        print(f"Base Fee: {base_fee:.4f}, Priority Fee: {priority_fee:.4f}, Total Fee: {total_fee:.4f}")
        return total_fee
