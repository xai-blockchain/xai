...
def create_founder_wallets():
    ...
    print(
        f"Founder (immediate): {FOUNDER_IMMEDIATE:>15,.0f} AIXN ({FOUNDER_IMMEDIATE/TOTAL_SUPPLY*100:.1f}%)"
    )
    print(
        f"Founder (vested):    {FOUNDER_VESTED:>15,.0f} AIXN ({FOUNDER_VESTED/TOTAL_SUPPLY*100:.1f}%)"
    )
    print(f"Development Fund:    {DEV_FUND:>15,.0f} AIXN ({DEV_FUND/TOTAL_SUPPLY*100:.1f}%)")
    print(
        f"Marketing Fund:      {MARKETING_FUND:>15,.0f} AIXN ({MARKETING_FUND/TOTAL_SUPPLY*100:.1f}%)"
    )
