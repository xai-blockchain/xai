...
class MultiSigTreasury:
    def __init__(self, owners: List[str], threshold: int):
        ...
        if not isinstance(threshold, int) or not (1 <= threshold <= len(owners)):
            raise ValueError(f"Threshold must be an integer between 1 and the number of owners ({len(owners)}).")
        ...
        print(f"MultiSigTreasury initialized with owners: {self.owners} and threshold: {self.threshold}-of-{len(self.owners)}.")
    ...
    def submit_transaction(self, proposer: str, recipient: str, amount: float) -> str:
        ...
        if amount > self.balance:
            raise ValueError(f"Insufficient treasury balance ({self.balance:.2f}) for transaction amount ({amount:.2f}).")
    ...
    def approve_transaction(self, approver: str, tx_id: str):
        ...
        print(f"Approver {approver} approved transaction {tx_id}. Current approvals: {len(transaction['approvals'])}/{self.threshold}")
    ...
    def execute_transaction(self, executor: str, tx_id: str):
        ...
        if len(transaction["approvals"]) < self.threshold:
            raise ValueError(f"Transaction {tx_id} does not have enough approvals. Required: {self.threshold}, Current: {len(transaction['approvals'])}")
        
        if transaction["amount"] > self.balance:
            raise ValueError(f"Insufficient treasury balance ({self.balance:.2f}) for transaction amount ({transaction['amount']:.2f}). Cannot execute.")

        self.balance -= transaction["amount"]
        transaction["executed"] = True
        print(f"Transaction {tx_id} executed by {executor}. Amount {transaction['amount']:.2f} sent to {transaction['recipient']}. New treasury balance: {self.balance:.2f}")
...
if __name__ == "__main__":
    ...
    try:
        tx3_id = treasury.submit_transaction("Eve", "0xGrant", 600.0) # Balance is 1000 - 200 - 300 = 500
    except ValueError as e:
        print(f"Error (expected): {e}")
