...
class SaltedHashManager:
    ...
    def verify_hash(self, data: str, stored_hash: str, stored_salt: str) -> bool:
        ...
        """
        if not isinstance(data, str) or not isinstance(stored_hash, str) or not isinstance(stored_salt, str):
...
if __name__ == "__main__":
    ...
    is_invalid_2 = hash_manager.verify_hash(user_password, hash1, salt2) # Using salt from another_password
    print(f"Verify '{user_password}' against Hash 1 with wrong salt (expected False): {is_invalid_2}")
