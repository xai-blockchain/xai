...
class QuantumResistantCryptoManager:
    ...
    def select_algorithm(self, algorithm_name: str) -> bool:
        ...
        if algorithm_name in self.available_algorithms:
            ...
            print(
                f"Selected quantum-resistant algorithm: {algorithm_name} ({self.available_algorithms[algorithm_name]})"
            )
            return True
        else:
    ...
    def verify_signature(
        self, message: str, signature: str, public_key: str, algorithm: str
    ) -> bool:
        """
        ...
        if key_pair:
            ...
            if signature:
                ...
                is_valid = crypto_manager.verify_signature(
                    message, signature, key_pair["public_key"], "Dilithium"
                )
                print(f"Signature Valid: {is_valid}")

                # Test with invalid message
                invalid_message = "This is a modified quantum-resistant message."
                is_valid_invalid_msg = crypto_manager.verify_signature(
                    invalid_message, signature, key_pair["public_key"], "Dilithium"
                )
