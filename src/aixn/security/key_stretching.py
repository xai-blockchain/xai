...
class KeyStretchingManager:
    def __init__(self, salt_length_bytes: int = 16, iterations: int = 100000):
        ...
        self.csprng = CSPRNG()
        print(f"KeyStretchingManager initialized with salt length: {self.salt_length_bytes} bytes, iterations: {self.iterations}.")
    ...
    def verify_key(self, password: str, stored_derived_key: bytes, stored_salt: bytes, stored_iterations: int) -> bool:
        """
        Verifies if a given password matches a stored derived key using the provided salt and iterations.
        """
        if not isinstance(password, str):
            raise TypeError("Password must be a string.")
        if not isinstance(stored_derived_key, bytes) or not isinstance(stored_salt, bytes) or not isinstance(stored_iterations, int):
...
if __name__ == "__main__":
    ...
    is_invalid_2 = key_stretcher.verify_key(user_password, derived_key_2, salt, iterations) # Using wrong salt
