...
class CircuitBreaker:
    ...
    def state(self) -> CircuitBreakerState:
        ...
        if self._state == CircuitBreakerState.OPEN:
            ...
            if current_timestamp >= self._last_trip_timestamp + self.recovery_timeout_seconds:
                ...
                self._half_open_attempts = 0
                print(
                    f"Circuit Breaker '{self.name}' transitioned to {self._state.value} state (recovery timeout)."
                )
        return self._state
    ...
    def record_failure(self):
        if self.state == CircuitBreakerState.CLOSED:
            ...
            print(
                f"Circuit Breaker '{self.name}': Failure recorded. Count: {self._failure_count}/{self.failure_threshold}"
            )
    ...
    def record_success(self):
        if self.state == CircuitBreakerState.HALF_OPEN:
            ...
            print(
                f"Circuit Breaker '{self.name}': Success recorded in HALF_OPEN. Attempts: {self._half_open_attempts}/{self.half_open_test_limit}"
            )
    ...
    def allow_request(self) -> bool:
        if self.state == CircuitBreakerState.OPEN:
            ...
            if self._half_open_attempts < self.half_open_test_limit:
                ...
                print(
                    f"Circuit Breaker '{self.name}' is HALF_OPEN, test limit reached. Request BLOCKED."
                )
                return False
        else:  # CLOSED
            return True
