...
class TSSInterface(ABC):
    ...
    @abstractmethod
    def verify_threshold_signature(self, message_hash: bytes, signature: str, public_key: str) -> bool:
        """
...
class MockTSS(TSSInterface):
    ...
    def distributed_sign(self, message_hash: bytes, collected_signatures: List[Tuple[str, str]], threshold: int) -> str:
        """
        ...
        if len(collected_signatures) < threshold:
            raise ValueError(f"Insufficient signatures collected. Required: {threshold}, Got: {len(collected_signatures)}")
        
        valid_signatures = []
        for sig_hex, signer_pub_hex in collected_signatures:
            if signer_pub_hex in [pk for _, pk in self._participants_keys.values()]: # Check if signer is a known participant
                try:
                    public_key = serialization.load_pem_public_key(
                        bytes.fromhex(signer_pub_hex),
        ...
        if len(valid_signatures) >= threshold:
            ...
            raise ValueError(f"Failed to collect enough valid signatures. Valid: {len(valid_signatures)}, Required: {threshold}")

    def verify_threshold_signature(self, message_hash: bytes, combined_signature: str, public_key: str) -> bool:
        """
...
if __name__ == "__main__":
    ...
    p1_priv_key = serialization.load_pem_private_key(p1_priv_pem, password=None, backend=default_backend())
    ...
    p2_priv_key = serialization.load_pem_private_key(p2_priv_pem, password=None, backend=default_backend())
    ...
    p3_priv_key = serialization.load_pem_private_key(p3_priv_pem, password=None, backend=default_backend())
    ...
    try:
        threshold_signature = tss_instance.distributed_sign(message_hash, collected_signatures, threshold)
        print(f"Threshold Signature (mock): {threshold_signature[:60]}...")
        
        # 5. Verify the threshold signature
        is_valid = tss_instance.verify_threshold_signature(message_hash, threshold_signature, master_pub_key)
