...
from aixn.core.ai_node_operator_questioning import (AINodeOperatorQuestioning, QuestionType, QuestionPriority, QuestionStatus)
    ...
    def ask_question_and_wait(
        ...
        while True:
            ...
            if answer.get('error') == 'TIMEOUT_INSUFFICIENT_VOTES':
                ...
                print(f"\r   Progress: {answer['votes_received']}/{answer['min_required']} operators ({progress:.1f}%) | Time left: {time_left:.0f}s", end='', flush=True)
...
if __name__ == '__main__':
    ...
    question_id = executor.task_questions.get("task_cardano_swap", [])[-1] if "task_cardano_swap" in executor.task_questions else None
