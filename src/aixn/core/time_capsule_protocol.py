...
class TimeCapsuleProtocol:
    ...
    def is_wallet_time_capsule_eligible(self, wallet_data: dict) -> bool:
        ...
        return wallet_data.get('time_capsule_eligible', False) and \
               not wallet_data.get('time_capsule_claimed', False)
    ...
    def initiate_time_capsule(self, wallet_data: dict, user_accepted: bool = True) -> dict:
        ...
        if not self.is_wallet_time_capsule_eligible(wallet_data):
            return {
                'success': False,
                'error': 'Wallet not eligible for time capsule protocol'
            }
        ...
        if not self._update_reserve_balance(bonus_amount):
            return {
                'success': False,
                'error': 'Failed to update reserve balance'
            }
...
"""
    ...
    def check_unlock_eligibility(self, locked_address: str) -> dict:
        ...
        capsule = next((tc for tc in self.time_capsules if tc['locked_address'] == locked_address), None)
    ...
    def claim_time_capsule(self, locked_address: str) -> dict:
        ...
        if capsule['claimed']:
            return {
                'success': False,
                'error': 'Time capsule already claimed'
            }
