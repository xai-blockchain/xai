...
class SocialRecoveryManager:
    ...
    def setup_guardians(
        ...
        if owner_address in self.recovery_configs:
            raise ValueError("Recovery already configured for this address. Cancel existing config first.")
    ...
    def initiate_recovery(
        ...
        print(f"   - Executable at: {datetime.fromtimestamp(request['executable_at']).strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def execute_recovery(
        ...
        print(f"   - Executed at: {datetime.fromtimestamp(request['executed_at']).strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def remove_recovery_config(
        ...
        for request in self.recovery_requests.values():
            if request['owner_address'] == owner_address and request['status'] in ['pending', 'approved']:
                raise ValueError("Cannot remove config with pending recovery requests. Cancel requests first.")
...
if __name__ == "__main__":
    ...
    try:
        result = manager.setup_guardians(
            owner_address=owner,
            guardian_addresses=[guardian1, guardian2, guardian3],
            threshold=2
        )
    ...
    try:
        result = manager.vote_recovery(
            request_id=request_id,
            guardian_address=guardian2
        )
