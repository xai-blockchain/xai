...
class ExchangeWallet:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            'available_balances': {k: float(self.get_available_balance(k)) for k in self.balances.keys()}
        }
...
class ExchangeWalletManager:
    ...
    def execute_trade(self, buyer_address: str, seller_address: str,
        ...
        if buyer_wallet.get_available_balance(quote_currency) < quote_amt:
            return {
                'success': False,
                'error': f'Buyer insufficient {quote_currency} balance'
            }
        ...
        if seller_wallet.get_available_balance(base_currency) < base_amt:
            return {
                'success': False,
                'error': f'Seller insufficient {base_currency} balance'
            }
    ...
    def get_transaction_history(self, user_address: str, limit: int = 50) -> List[dict]:
        ...
        user_txs = [
            ...
            if tx.get('user_address') == user_address or
               tx.get('buyer') == user_address or
               tx.get('seller') == user_address
        ]
    ...
    def load_wallets(self):
        ...
        if os.path.exists(wallets_file):
            ...
            for address, data in wallets_data.items():
                ...
                wallet.locked_balances = {k: Decimal(str(v)) for k, v in data['locked_balances'].items()}
