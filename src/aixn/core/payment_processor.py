...
class PaymentProcessor:
    ...
    def calculate_purchase(self, usd_amount: float) -> Dict:
        ...
        if amount < self.min_purchase_usd:
            return {
                'success': False,
                'error': f'Minimum purchase is ${self.min_purchase_usd}'
            }
        if amount > self.max_purchase_usd:
            return {
                'success': False,
                'error': f'Maximum purchase is ${self.max_purchase_usd}'
            }
    ...
    def process_card_payment(self, user_address: str, usd_amount: float,
                             card_token: str, email: str) -> Dict:
    ...
    def process_bank_transfer(self, user_address: str, usd_amount: float,
        ...
        if net_amount < 0:
            return {
                'success': False,
                'error': 'Bank transfer amount too small to cover fee'
            }
    ...
    def get_supported_payment_methods(self) -> Dict:
        fee_text = f"{float(self.credit_card_fee_percent * 100):.1f}% + ${self.credit_card_fee_fixed}"
        return {
            'credit_card': {
                'enabled': True,
                'fee': fee_text,
