...
from aixn.core.monitoring import MetricsCollector, AlertLevel
from aixn.core.structured_logger import StructuredLogger, LogContext, PerformanceTimer
import time
...
def example_blockchain_integration():
    ...
    for i in range(3):
        mine_block(
            block_index=i + 1,
            miner_address='XAI1234567890abcdef',
            tx_count=5
        )
        time.sleep(0.2)
    ...
    handle_security_event(
        'rate_limit_exceeded',
        'WARN',
        {'endpoint': '/send', 'requests_per_minute': 120}
    )
