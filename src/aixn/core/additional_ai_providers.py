...
def extend_executor_with_new_providers(executor_class):
    ...
    # Add methods to executor
    def _call_perplexity_with_limit(self, api_key, task, max_tokens):
        return perplexity.call_with_limit(api_key, task, max_tokens)

    def _call_groq_with_limit(self, api_key, task, max_tokens):
        return groq.call_with_limit(api_key, task, max_tokens)

    def _call_xai_with_limit(self, api_key, task, max_tokens):
        return xai.call_with_limit(api_key, task, max_tokens)

    def _call_together_with_limit(self, api_key, task, max_tokens):
        return together.call_with_limit(api_key, task, max_tokens)

    def _call_fireworks_with_limit(self, api_key, task, max_tokens):
        return fireworks.call_with_limit(api_key, task, max_tokens)

    def _call_deepseek_with_limit(self, api_key, task, max_tokens):
        return deepseek.call_with_limit(api_key, task, max_tokens)
