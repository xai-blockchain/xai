...
class MiningBonusManager:
    ...
    def register_miner(self, address: str) -> Dict:
        ...
        if early_adopter_bonus > 0:
            ...
            self._award_bonus(address, 'early_adopter', early_adopter_bonus,
    ...
    def check_achievements(self, address: str, blocks_mined: int, streak_days: int) -> Dict:
        ...
        if blocks_mined >= 1 and 'first_block' not in [a['type'] for a in self.achievements[address]['earned']]:
        ...
        if blocks_mined >= 10 and 'ten_blocks' not in [a['type'] for a in self.achievements[address]['earned']]:
        ...
        if blocks_mined >= 100 and 'hundred_blocks' not in [a['type'] for a in self.achievements[address]['earned']]:
        ...
        if streak_days >= 7 and 'seven_day_streak' not in [a['type'] for a in self.achievements[address]['earned']]:
    ...
    def use_referral_code(self, new_address: str, referral_code: str) -> Dict:
        ...
        self._award_bonus(referrer_address, 'referral', referral_bonus,
    ...
    def check_referral_milestone(self, referrer_address: str, referred_address: str, blocks_mined: int) -> Dict:
        ...
        if blocks_mined >= 10 and not referred_record.get('milestone_bonus_claimed', False):
            ...
            self._award_bonus(referrer_address, 'referral_milestone', milestone_bonus,
        ...
        return {
            'success': False,
            'message': 'Milestone not yet reached or already claimed'
        }
    ...
    def get_stats(self) -> Dict:
        ...
        total_bonuses_awarded = sum(
            data.get('total_awarded', 0)
            for data in self.bonuses.values()
        )

        active_referrals = sum(
            1 for data in self.referrals.values()
    ...
    def update_miner_stats(self, address: str, blocks_mined: int,
                          streak_days: int, mining_reward: float) -> None:
