...
class TransactionValidator:
    ...
    def __init__(self, blockchain, nonce_tracker: Optional[NonceTracker] = None, logger: Optional[StructuredLogger] = None, utxo_manager: Optional[UTXOManager] = None):
    ...
    def validate_transaction(self, transaction: 'Transaction', is_mempool_check: bool = True) -> bool:
        """
        ...
        try:
            ...
            if not all(hasattr(transaction, attr) for attr in ['sender', 'recipient', 'amount', 'fee', 'timestamp', 'signature', 'txid', 'inputs', 'outputs']):
            ...
            self.security_validator.validate_hex_string(transaction.signature, "signature", exact_length=128)
            self.security_validator.validate_hex_string(transaction.txid, "transaction ID", exact_length=64)

            # 3. Transaction ID (hash) verification
            if transaction.calculate_hash() != transaction.txid:
                raise ValidationError("Transaction ID mismatch. Transaction data has been tampered with.")
            ...
            if transaction.tx_type != "coinbase":
                ...
                for i, tx_input in enumerate(transaction.inputs):
                    if not all(k in tx_input for k in ['txid', 'vout']):
                        raise ValidationError(f"Transaction input {i} is missing required fields (txid, vout).")
                    
                    utxo = self.utxo_manager.get_unspent_output(tx_input['txid'], tx_input['vout'])
                    if not utxo:
                        raise ValidationError(f"Transaction input {tx_input['txid']}:{tx_input['vout']} is not an unspent UTXO.")
                    ...
                    if utxo['script_pubkey'] != f"P2PKH {transaction.sender}": # Assuming P2PKH format
                         raise ValidationError(f"Transaction input {tx_input['txid']}:{tx_input['vout']} does not belong to sender {transaction.sender}.")
                ...
                for i, tx_output in enumerate(transaction.outputs):
                    if not all(k in tx_output for k in ['address', 'amount']):
                        raise ValidationError(f"Transaction output {i} is missing required fields (address, amount).")
                    self.security_validator.validate_address(tx_output['address'], f"output {i} address")
                    self.security_validator.validate_amount(tx_output['amount'], f"output {i} amount")
                    output_sum += tx_output['amount']

                # Verify input sum covers output sum + fee
                if input_sum < (output_sum + transaction.fee):
                    raise ValidationError(f"Insufficient input funds. Input sum: {input_sum}, Output sum: {output_sum}, Fee: {transaction.fee}")
            ...
            if not self.nonce_tracker.validate_nonce(transaction.sender, transaction.nonce):
                raise ValidationError(f"Invalid nonce for sender {transaction.sender}. Expected: {self.nonce_tracker.get_next_nonce(transaction.sender)}, Got: {transaction.nonce}")
            ...
            self.logger.debug(f"Transaction {transaction.txid[:10]}... is valid.", txid=transaction.txid)
            return True

        except ValidationError as e:
            self.logger.warn(f"Transaction validation failed for {transaction.txid[:10]}...: {e}", txid=transaction.txid, error=str(e))
            return False
        except Exception as e:
            self.logger.error(f"An unexpected error occurred during transaction validation for {transaction.txid[:10]}...: {e}", txid=transaction.txid, error=str(e), exc_info=True)
            return False
    ...
    def _validate_time_capsule_lock(self, transaction: 'Transaction'):
        ...
        if 'unlock_time' not in transaction.metadata or not isinstance(transaction.metadata['unlock_time'], (int, float)):
    ...
    def _validate_governance_vote(self, transaction: 'Transaction'):
        ...
        if 'vote' not in transaction.metadata or transaction.metadata['vote'] not in ['yes', 'no', 'abstain']:
...
def get_transaction_validator(blockchain, nonce_tracker: Optional[NonceTracker] = None, logger: Optional[StructuredLogger] = None, utxo_manager: Optional[UTXOManager] = None) -> TransactionValidator:
    """
    Get global transaction validator instance.
    """
    global _global_transaction_validator
    if _global_transaction_validator is None:
        _global_transaction_validator = TransactionValidator(blockchain, nonce_tracker, logger, utxo_manager)
