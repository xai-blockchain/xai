...
class BlockchainMetrics:
    ...
    def __init__(self, port: int = 8000, registry: Optional[CollectorRegistry] = None):
        ...
        self.blocks_total = Counter(
            "aixn_blocks_total", "Total number of blocks mined", registry=self.registry
        )

        self.block_height = Gauge(
            "aixn_block_height", "Current blockchain height", registry=self.registry
        )
        ...
        self.block_difficulty = Gauge(
            "aixn_block_difficulty", "Current mining difficulty", registry=self.registry
        )
        ...
        self.peers_connected = Gauge(
            "aixn_peers_connected", "Number of connected peers", registry=self.registry
        )
        ...
        self.total_supply = Gauge(
            "aixn_total_supply_aixn", "Total AIXN supply", registry=self.registry
        )

        self.circulating_supply = Gauge(
            "aixn_circulating_supply_aixn", "Circulating AIXN supply", registry=self.registry
        )
        ...
        self.mining_attempts = Counter(
            "aixn_mining_attempts_total", "Total mining attempts", registry=self.registry
        )

        self.mining_success = Counter(
            "aixn_mining_success_total", "Successful mining operations", registry=self.registry
        )

        # ==================== SYSTEM METRICS ====================
        self.system_cpu_usage = Gauge(
            "aixn_system_cpu_usage_percent", "CPU usage percentage", registry=self.registry
        )

        self.system_memory_usage = Gauge(
            "aixn_system_memory_usage_bytes", "Memory usage in bytes", registry=self.registry
        )

        self.system_memory_percent = Gauge(
            "aixn_system_memory_percent", "Memory usage percentage", registry=self.registry
        )

        self.system_disk_usage = Gauge(
            "aixn_system_disk_usage_bytes", "Disk usage in bytes", registry=self.registry
        )

        self.system_disk_percent = Gauge(
            "aixn_system_disk_percent", "Disk usage percentage", registry=self.registry
        )

        self.process_uptime_seconds = Gauge(
            "aixn_process_uptime_seconds", "Process uptime in seconds", registry=self.registry
        )
        ...
        self.orphaned_blocks = Counter(
            "aixn_orphaned_blocks_total", "Total orphaned blocks", registry=self.registry
        )

        self.reorgs = Counter(
            "aixn_chain_reorgs_total", "Total chain reorganizations", registry=self.registry
        )
        ...
        # ==================== INFO METRICS ====================
        self.node_info = Info("aixn_node", "AIXN node information", registry=self.registry)
