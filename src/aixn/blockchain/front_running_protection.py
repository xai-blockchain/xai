...
class FrontRunningProtectionManager:
    def __init__(self):
        self.committed_transactions: Dict[str, Dict[str, Any]] = {} # {commit_hash: {"sender": str, "revealed_tx": dict, "salt": str, "status": str}}
    ...
    def reveal_transaction(self, sender_address: str, transaction: Dict[str, Any], salt: str):
        ...
        if not committed_entry:
            raise ValueError(f"No matching commit found for computed hash {computed_commit_hash[:10]}...")
        ...
        tx_hash_for_processing = self._hash_transaction_with_salt(transaction, salt) # Use the full hash for internal tracking
    ...
    def check_slippage(self, requested_amount: float, actual_amount: float, max_slippage_percent: float) -> bool:
        """
        ...
        if not isinstance(max_slippage_percent, (int, float)) or not (0 <= max_slippage_percent < 100):
        ...
        if slippage > max_slippage_percent:
            print(f"Slippage check FAILED: Requested {requested_amount}, Actual {actual_amount}, "
                  f"Slippage {slippage:.2f}%, Max allowed {max_slippage_percent:.2f}%.")
            return False
        else:
            print(f"Slippage check PASSED: Requested {requested_amount}, Actual {actual_amount}, "
                  f"Slippage {slippage:.2f}%, Max allowed {max_slippage_percent:.2f}%.")
            return True
...
if __name__ == "__main__":
    ...
    tx_a_original = {"type": "swap", "token_in": "ETH", "amount_in": 1.0, "token_out": "USDC", "min_amount_out": 1950.0}
