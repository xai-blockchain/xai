...
class InflationMonitor:
    def __init__(self, token_supply_manager: TokenSupplyManager, alert_threshold_percentage: float = 5.0, history_window_days: int = 30):
        ...
        if not isinstance(alert_threshold_percentage, (int, float)) or not (0 <= alert_threshold_percentage <= 100):
        ...
        print(f"InflationMonitor initialized. Alert threshold: {self.alert_threshold_percentage:.2f}%, History window: {history_window_days} days.")
    ...
    def _record_current_supply(self):
        ...
        while self.supply_history and (current_time - self.supply_history[0][0]) > self.history_window_seconds:
    ...
    def check_for_alerts(self, period_seconds: int) -> bool:
        ...
        if inflation_rate > self.alert_threshold_percentage:
            print(f"!!! INFLATION ALERT !!! Inflation rate ({inflation_rate:.2f}%) "
