...
class TransferTaxManager:
    def __init__(
        ...
        print(
            f"TransferTaxManager initialized. Tax rate: {self.transfer_tax_rate_percentage}%. Exempt addresses: {self.exempt_addresses}"
        )
    ...
    def apply_transfer_tax(self, sender: str, recipient: str, amount: float) -> Tuple[float, float]:
        ...
        print(
            f"Applied {self.transfer_tax_rate_percentage}% transfer tax. Original: {amount:.2f}, Tax: {tax_amount:.2f}, Net: {net_amount:.2f}"
        )
...
if __name__ == "__main__":
    ...
    tax_manager = TransferTaxManager(
        transfer_tax_rate_percentage=1.0, exempt_addresses=exempt_list
    )  # 1% tax
