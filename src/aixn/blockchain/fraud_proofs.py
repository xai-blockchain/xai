...
class FraudProof:
    def __init__(
        ...
        if challenge_period_ends_at <= datetime.now(timezone.utc).timestamp():
            print(
                "Warning: Challenge period ends in the past or immediately. Ensure future timestamp."
            )
...
class FraudProofManager:
    ...
    def submit_fraud_proof(
        ...
        challenge_period_ends_at = (
            int(datetime.now(timezone.utc).timestamp()) + challenge_period_duration_seconds
        )
        ...
        print(
            f"Fraud proof {proof_id} submitted by {challenger_address} against {challenged_validator_address} for block {block_number}."
        )
        return proof
    ...
    def verify_fraud_proof(self, proof_id: str) -> bool:
        ...
        if is_valid:
            ...
            print(
                f"Fraud proof {proof_id} VERIFIED. Slashing challenged validator {proof.challenged_validator_address}."
            )
...
if __name__ == "__main__":
    ...
    proof1 = fraud_manager.submit_fraud_proof(
        ...
        proof_data={
            ...
            "details": "State hash mismatch at block 123",
        },
        block_number=123,
    ...
    print(
        f"Validator 0xChallengedValidator stake after verification: {slashing_manager.get_validator_stake('0xChallengedValidator').staked_amount}"
    )

    # Verify proof 2 (should be rejected due to simplified logic)
    fraud_manager.verify_fraud_proof(proof2._proof_counter)
    print(
        f"Validator 0xHonestValidator stake after verification: {slashing_manager.get_validator_stake('0xHonestValidator').staked_amount}"
    )
    ...
    proof3 = fraud_manager.submit_fraud_proof(
        ...
        proof_data={
            "type": "incorrect_signature_in_block",
            "details": "Signature for tx 789 is invalid",
        },
        block_number=789,
    ...
    print(
        f"Validator 0xValidatorA stake after expired proof attempt: {slashing_manager.get_validator_stake('0xValidatorA').staked_amount}"
    )
