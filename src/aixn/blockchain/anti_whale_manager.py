...
class AntiWhaleManager:
    def __init__(
        ...
        print(
            f"AntiWhaleManager initialized. Max voting power: {self.max_governance_voting_power_percentage}%, Max transaction size: {self.max_transaction_size_percentage_of_total_supply}% of total supply."
        )
    ...
    def check_governance_vote(self, voter_address: str, proposed_vote_power: float) -> float:
        ...
        if effective_vote_power < proposed_vote_power:
            print(
                f"Warning: Voter {voter_address} proposed {proposed_vote_power:.2f} voting power, "
    ...
    def check_transaction_size(self, sender_address: str, transaction_amount: float) -> bool:
        ...
        if total_supply == 0:
            print(
                f"Transaction from {sender_address} for {transaction_amount:.2f} blocked: Total supply is zero."
            )
            return False
        ...
        if transaction_amount > max_allowed_transaction_amount:
            print(
                f"!!! ANTI-WHALE ALERT !!! Transaction from {sender_address} for {transaction_amount:.2f} "
...
if __name__ == "__main__":
    ...
    anti_whale_manager = AntiWhaleManager(
        supply_manager,
        max_governance_voting_power_percentage=5.0,  # 5% cap
        max_transaction_size_percentage_of_total_supply=0.5,
    )  # 0.5% cap
    ...
    print(
        f"Whale effective vote power: {effective_vote_whale:.2f}"
    )  # Should be capped at 5% of 800k = 40k
    ...
    # Normal transaction
    is_tx_allowed_normal = anti_whale_manager.check_transaction_size(
        normal_user, 3000.0
    )  # 3000 is less than 0.5% of 800k (4000)
