...
class VestingManager:
    ...
    def create_vesting_schedule(
        ...
        print(
            f"Vesting schedule {schedule_id} created for {recipient_address} for {total_amount:.2f} tokens."
        )
        return schedule_id
    ...
    def claim_vested_tokens(self, schedule_id: str, current_time: Optional[int] = None) -> float:
        ...
        print(
            f"Claimed {available_to_claim:.2f} tokens for schedule {schedule_id}. Total claimed: {schedule['claimed_amount']:.2f}"
        )
...
if __name__ == "__main__":
    ...
    schedule_id_investor = manager.create_vesting_schedule(
        investor, 5000.0, start_t, start_t + 60, 20
    )
    ...
    print(
        f"Team member vested (after claim): {manager.get_vested_amount(schedule_id_team, current_t1):.2f}"
    )
    ...
    print(
        f"Team member vested (after full claim): {manager.get_vested_amount(schedule_id_team, current_t3):.2f}"
    )
