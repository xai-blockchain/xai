...
class WashTradingDetector:
    ...
    def record_trade(
        ...
        print(
            f"Recorded trade {trade_id}: {asset} {amount} @ {price} from {seller_address} to {buyer_address}"
        )
    ...
    def detect_self_trading(self) -> bool:
        ...
        for trade in self.trade_history:
            if trade["buyer"] == trade["seller"]:
                print(
                    f"!!! WASH TRADING DETECTED: Self-trading in trade {trade['trade_id']} by {trade['buyer']}."
                )
            ...
            for i in range(len(recent_trades)):
                ...
                for j in range(i + 1, len(recent_trades)):
                    ...
                    if (
                        ...
                        print(
                            f"!!! WASH TRADING DETECTED: Round-trip trading of {asset} between {trade1['seller']} and {trade1['buyer']} in trades {trade1['trade_id']} and {trade2['trade_id']}."
                        )
                        self.circuit_breaker.record_failure()
                        return True
...
if __name__ == "__main__":
    ...
    detector = WashTradingDetector(
        cb, round_trip_time_window_seconds=10
    )  # 10-second window for example
    ...
    print(
        f"Wash trading detected: {detector.detect_self_trading() or detector.detect_round_trip_trading()}"
    )
    ...
    cb_rt = CircuitBreaker(
        name="WashTradingCB_RT", failure_threshold=1, recovery_timeout_seconds=60
    )
    ...
    time.sleep(2)
    detector_rt.record_trade(
        user_b, user_a, asset_eth, 1.0, 2011.0
    )  # User B buys back from A (round trip)
    print(
        f"Wash trading detected: {detector_rt.detect_self_trading() or detector_rt.detect_round_trip_trading()}"
    )
    print(f"Circuit Breaker State: {cb_rt.state}\n")

    print("--- Scenario 4: Round-Trip Trading (outside time window) ---")
    # Reset CB for this scenario
    cb_rt_long = CircuitBreaker(
        name="WashTradingCB_RT_Long", failure_threshold=1, recovery_timeout_seconds=60
    )
    ...
    print(
        f"Wash trading detected: {detector_rt_long.detect_self_trading() or detector_rt_long.detect_round_trip_trading()}"
    )
