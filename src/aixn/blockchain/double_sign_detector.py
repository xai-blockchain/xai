...
class DoubleSignDetector:
    ...
    def _generate_double_sign_proof(self, validator_id: str, block_height: int,
                                    first_signed_block_hash: str, second_signed_block_hash: str) -> Dict[str, Any]:
        """
        ...
        proof = {
            ...
            "conflicting_signatures": [
                {"block_hash": first_signed_block_hash, "signature": f"sig_for_{first_signed_block_hash}"},
                {"block_hash": second_signed_block_hash, "signature": f"sig_for_{second_signed_block_hash}"}
            ],
            "timestamp": time.time()
        }
    ...
    def process_signed_block(self, validator_id: str, block_height: int, signed_block_hash: str) -> Tuple[bool, Dict[str, Any] | None]:
        """
        ...
        if block_height in validator_blocks:
            ...
            if existing_signed_block_hash != signed_block_hash:
                ...
                print(f"!!! DOUBLE-SIGN DETECTED !!! Validator {validator_id} signed two different blocks "
                      f"at height {block_height}: {existing_signed_block_hash[:8]}... and {signed_block_hash[:8]}...")
                proof = self._generate_double_sign_proof(validator_id, block_height,
                                                         existing_signed_block_hash, signed_block_hash)
                return True, proof
            else:
                # Same block signed again (e.g., re-broadcast), not a double-sign
                print(f"Validator {validator_id} re-signed block {signed_block_hash[:8]}... at height {block_height}.")
                return False, None
        else:
            # First time this validator signed a block at this height
            validator_blocks[block_height] = signed_block_hash
            print(f"Validator {validator_id} signed block {signed_block_hash[:8]}... at height {block_height}.")
            return False, None
...
if __name__ == "__main__":
    ...
    print(f"Double-sign detected: {is_ds}")
    is_ds, proof = detector.process_signed_block("validator_Z", 102, "0xblock102_B") # Different hash at same height
    print(f"Double-sign detected: {is_ds}")
    if is_ds:
        print("Proof:", proof)
