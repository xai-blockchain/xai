...
class TombstoneManager:
    def __init__(self, initial_validators: Dict[str, float], tombstone_threshold: int = 3):
        ...
        print(
            f"TombstoneManager initialized with {len(self.active_validators)} active validators. "
    ...
    def record_slashing_event(self, validator_id: str, slashed_amount: float):
        ...
        if validator_id in self.active_validators:
            ...
            print(
                f"Slashing event recorded for {validator_id}. New staked: {validator_info['staked_amount']:.2f}, "
                f"Slashed count: {validator_info['slashed_count']}."
            )
            self.check_for_tombstone(validator_id)
        else:
            print(
                f"Warning: Slashing event recorded for unknown or inactive validator {validator_id}."
            )
    ...
    def check_for_tombstone(self, validator_id: str) -> bool:
        ...
        if validator_id in self.active_validators:
            ...
            if validator_info["slashed_count"] >= self.tombstone_threshold:
                print(
                    f"Validator {validator_id} reached {self.tombstone_threshold} slashing events. Initiating tombstoning."
                )
    ...
    def tombstone_validator(self, validator_id: str):
        ...
        if validator_id in self.active_validators:
            validator_info = self.active_validators[validator_id]
            print(
                f"!!! TOMBSTONING VALIDATOR {validator_id} !!! "
...
if __name__ == "__main__":
    initial_stakes = {"validator_A": 1000.0, "validator_B": 1500.0, "validator_C": 2000.0}
    manager = TombstoneManager(
        initial_stakes, tombstone_threshold=2
    )  # Tombstone after 2 slashing events
