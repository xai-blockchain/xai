...
class InsuranceFundManager:
    ...
    def submit_claim(self, user_address: str, amount_claimed: float, description: str) -> str:
        ...
        print(f"Claim {claim_id} submitted by {user_address} for {amount_claimed:.4f}. Status: pending.")
        return claim_id
    ...
    def approve_claim(self, claim_id: str, approver_address: str):
        ...
        if claim["status"] != "pending":
            raise ValueError(f"Claim {claim_id} is not in pending status (current: {claim['status']}).")
        
        # For simplicity, any authorized payout address can approve
        if approver_address != self.authorized_payout_address:
            raise PermissionError(f"Approver {approver_address} is not authorized to approve claims.")
    ...
    def process_payout(self, claim_id: str, caller_address: str):
        ...
        if claim["status"] != "approved":
            raise ValueError(f"Claim {claim_id} is not in approved status (current: {claim['status']}).")
        ...
        if self.fund_balance < amount_to_pay:
            print(f"Warning: Insurance fund balance ({self.fund_balance:.4f}) is insufficient to cover claim {claim_id} ({amount_to_pay:.4f}).")
        ...
        claim["status"] = "paid"
        print(f"Payout of {amount_to_pay:.4f} processed for claim {claim_id} to {claim['user_address']}. "
...
if __name__ == "__main__":
    ...
    insurance_manager = InsuranceFundManager(fee_manager, authorized_payout_address="0xInsuranceDAO")
    ...
    print("\n--- Submitting Claims ---")
    claim1_id = insurance_manager.submit_claim("0xVictim1", 75.0, "Lost 75 tokens due to bridge exploit in Block 123.")
    claim2_id = insurance_manager.submit_claim("0xVictim2", 100.0, "Lost 100 tokens due to bridge exploit in Block 124.")
    claim3_id = insurance_manager.submit_claim("0xVictim3", 200.0, "Lost 200 tokens due to bridge exploit in Block 125.")
