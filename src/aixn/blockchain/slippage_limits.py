...
class LiquidityPool:
    def __init__(self, token_x_reserve: float, token_y_reserve: float, 
                 pool_slippage_limit_percentage: float = 0.5, 
                 max_trade_size: float = float('inf'),
                 price_impact_rejection_threshold_percentage: float = 2.0): # Default 2%
        ...
        if not isinstance(pool_slippage_limit_percentage, (int, float)) or not (0 <= pool_slippage_limit_percentage < 100):
            raise ValueError("Pool slippage limit percentage must be between 0 and 100 (exclusive of 100).")
        if not isinstance(max_trade_size, (int, float)) or max_trade_size <= 0:
            raise ValueError("Max trade size must be a positive number.")
        if not isinstance(price_impact_rejection_threshold_percentage, (int, float)) or not (0 <= price_impact_rejection_threshold_percentage < 100):
            raise ValueError("Price impact rejection threshold must be between 0 and 100 (exclusive of 100).")
        ...
        self.price_impact_rejection_threshold_percentage = price_impact_rejection_threshold_percentage
    ...
    def calculate_price_impact(self, amount_in: float, token_in: str) -> float:
        ...
        initial_price = self.reserve_y / self.reserve_x if token_in == "X" else self.reserve_x / self.reserve_y
    ...
    def swap(self, amount_in: float, token_in: str, min_amount_out: float = 0.0) -> float:
        ...
        if amount_in > self.max_trade_size:
            raise ValueError(f"Swap failed: Amount in {amount_in:.4f} exceeds maximum allowed trade size of {self.max_trade_size:.4f}.")
        ...
        if price_impact > self.price_impact_rejection_threshold_percentage:
            raise ValueError(f"Swap failed: Price impact {price_impact:.2f}% exceeds rejection threshold of {self.price_impact_rejection_threshold_percentage:.2f}%.")
        ...
        if actual_output_raw < min_amount_out:
            raise ValueError(f"Swap failed: Actual output {actual_output_raw:.4f} is less than user's minimum acceptable output {min_amount_out:.4f}.")
        ...
        if slippage_percentage > self.pool_slippage_limit_percentage:
            raise ValueError(f"Swap failed: Slippage {slippage_percentage:.2f}% exceeds pool's limit of {self.pool_slippage_limit_percentage:.2f}%.")
        ...
        print(f"Swap successful: {amount_in:.4f} {token_in} for {actual_output_raw:.4f} {('Y' if token_in == 'X' else 'X')}. "
...
if __name__ == "__main__":
    ...
    pool = LiquidityPool(token_x_reserve=1000.0, token_y_reserve=1000000.0, 
                         pool_slippage_limit_percentage=1.0, max_trade_size=100.0,
                         price_impact_rejection_threshold_percentage=1.5) # 1.5% price impact limit
    ...
    try:
        ...
        pool_large_swap_test = LiquidityPool(token_x_reserve=1000.0, token_y_reserve=1000000.0, 
                                             pool_slippage_limit_percentage=0.5, max_trade_size=100.0,
        ...
        print(f"Error: {e}")
    print(f"Current Reserves (large swap test): X={pool_large_swap_test.reserve_x:.4f}, Y={pool_large_swap_test.reserve_y:.4f}")

    print("\n--- Scenario 4: Swap exceeding maximum trade size ---")
    try:
        pool.swap(amount_in=150.0, token_in="X", min_amount_out=0.0) # Try to swap 150 ETH (max is 100)
    except ValueError as e:
        print(f"Error: {e}")
    print(f"Current Reserves: X={pool.reserve_x:.4f}, Y={pool.reserve_y:.4f}")
    ...
    try:
        # Reset pool for this test
        pool_price_impact_test = LiquidityPool(token_x_reserve=1000.0, token_y_reserve=1000000.0, 
                                               pool_slippage_limit_percentage=1.0, max_trade_size=100.0,
                                               price_impact_rejection_threshold_percentage=0.1) # Very low threshold
        pool_price_impact_test.swap(amount_in=50.0, token_in="X", min_amount_out=0.0) # This should exceed 0.1% impact
    except ValueError as e:
        print(f"Error: {e}")
    print(f"Current Reserves (price impact test): X={pool_price_impact_test.reserve_x:.4f}, Y={pool_price_impact_test.reserve_y:.4f}")
