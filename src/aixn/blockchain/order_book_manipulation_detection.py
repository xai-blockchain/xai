...
class OrderBook:
    ...
    def place_order(self, order_type: str, price: float, amount: float) -> str:
...
if __name__ == "__main__":
    order_book = OrderBook()
    cb = CircuitBreaker(name="OrderBookCB", failure_threshold=1, recovery_timeout_seconds=60)
    detector = OrderBookManipulationDetector(order_book, cb)

    print("--- Scenario 1: Normal Orders ---")
    order_book.place_order("buy", 100.0, 10)
    order_book.place_order("sell", 101.0, 10)
    order_book.place_order("buy", 99.5, 5)
    order_book.place_order("sell", 101.5, 8)
