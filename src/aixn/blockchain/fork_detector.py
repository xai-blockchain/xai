...
class ForkDetector:
    def __init__(self, node_id: str, initial_chain_head: Dict[str, Any]):
        ...
        if not isinstance(initial_chain_head, dict) or "hash" not in initial_chain_head or "height" not in initial_chain_head:
        ...
        print(f"ForkDetector initialized for node {self.node_id}. Initial chain head: {self.current_chain_head['height']} ({self.current_chain_head['hash'][:8]}...)")

    def update_node_chain_head(self, new_chain_head: Dict[str, Any]):
        """Updates the node's own view of the current chain head."""
        if not isinstance(new_chain_head, dict) or "hash" not in new_chain_head or "height" not in new_chain_head:
            raise ValueError("New chain head must be a dictionary with 'hash' and 'height'.")
        self.current_chain_head = new_chain_head
        print(f"Node {self.node_id} updated chain head to: {self.current_chain_head['height']} ({self.current_chain_head['hash'][:8]}...)")
    ...
    def report_peer_chain_head(self, peer_id: str, peer_chain_head: Dict[str, Any]):
        ...
        if not isinstance(peer_chain_head, dict) or "hash" not in peer_chain_head or "height" not in peer_chain_head:
            raise ValueError("Peer chain head must be a dictionary with 'hash' and 'height'.")
        self.peer_chain_heads[peer_id] = peer_chain_head
        print(f"Peer {peer_id} reported chain head: {peer_chain_head['height']} ({peer_chain_head['hash'][:8]}...)")
    ...
    def check_for_forks(self, consensus_threshold: float = 0.51) -> bool:
        ...
        for peer_id, peer_head in self.peer_chain_heads.items():
            if peer_head["hash"] == self.current_chain_head["hash"] and \
               peer_head["height"] == self.current_chain_head["height"]:
                agreeing_peers += 1
        ...
        if agreement_percentage < consensus_threshold:
            print(f"!!! FORK ALERT !!! Node {self.node_id} has {agreement_percentage:.2%} agreement with peers on chain head. "
            ...
            print(f"Node {self.node_id} is in consensus with {agreement_percentage:.2%} of peers. No fork detected.")
            return False
...
if __name__ == "__main__":
    ...
    node_a_detector.check_for_forks(consensus_threshold=0.6) # 3/7 = ~42% agreement, should trigger alert
    ...
    node_a_detector.check_for_forks(consensus_threshold=0.6) # 3/7 = ~42% agreement, still a risk, but Node A is on the longer chain
