...
from src.aixn.security.circuit_breaker import (
    CircuitBreaker,
    CircuitBreakerState,
    ...
    def check_and_auto_pause(self):
        ...
        if self.circuit_breaker and self.circuit_breaker.state == CircuitBreakerState.OPEN:
            if not self._is_paused:
                ...
                print(
                    f"Circuit Breaker '{self.circuit_breaker.name}' is OPEN, but operations already paused."
                )
        elif (
            self.circuit_breaker
            and self.circuit_breaker.state == CircuitBreakerState.CLOSED
            and self._is_paused
            and self.paused_by == "0xAutomatedSystem"
        ):
...
if __name__ == "__main__":
    ...
    bridge_cb = CircuitBreaker(
        name="BridgeAnomalyDetector", failure_threshold=2, recovery_timeout_seconds=5
    )
    ...
    pause_manager.pause_operations(
        AUTHORIZED_ADMIN, "Critical vulnerability detected in bridge contract."
    )
    print(pause_manager.get_status())
