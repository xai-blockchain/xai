...
class CrossChainMessageVerifier:
    ...
    def verify_message(self, message: CrossChainMessage, origin_chain_merkle_root: str) -> bool:
        ...
        if not message.merkle_proof:
            print(
                f"Error: Message {message.sequence_number} from {message.origin_chain_id} has no Merkle proof attached."
            )
            return False
        ...
        if is_valid:
            print(
                f"Cross-chain message {message.sequence_number} from {message.origin_chain_id} VERIFIED."
            )
        else:
            print(
                f"Cross-chain message {message.sequence_number} from {message.origin_chain_id} FAILED VERIFICATION."
            )
...
if __name__ == "__main__":
    ...
    origin_chain_messages_data = [
        {
            ...
            "recipient_address": "0xReceiver1",
            "payload": {"type": "transfer", "amount": 100},
            "sequence_number": 1,
        },
        {
            "origin_chain_id": "ChainA",
            ...
            "recipient_address": "0xReceiver2",
            "payload": {"type": "call_contract", "contract": "0xContractX", "method": "mint"},
            "sequence_number": 2,
        },
        {
            "origin_chain_id": "ChainA",
            ...
            "recipient_address": "0xReceiver3",
            "payload": {"type": "transfer", "amount": 50},
            "sequence_number": 3,
        },
    ]
    ...
    invalid_proof = origin_merkle_tree.generate_merkle_proof(
        invalid_proof_message_data
    )  # Proof for message 2
