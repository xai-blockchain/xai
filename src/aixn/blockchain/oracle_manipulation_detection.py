...
class OracleManipulationDetector:
    def __init__(self, twap_oracle: TWAPOracle, circuit_breaker: CircuitBreaker,
                 deviation_threshold_percentage: float = 5.0): # 5% deviation
        ...
        if not isinstance(deviation_threshold_percentage, (int, float)) or not (0 <= deviation_threshold_percentage < 100):
    ...
    def check_for_manipulation(self, current_prices: Dict[str, float], current_timestamp: int = None) -> bool:
        """
        ...
        if len(prices_list) > 1:
            ...
            if avg_price > 0: # Avoid division by zero
                ...
                if max_deviation_from_avg > self.deviation_threshold_percentage:
                    print(f"!!! ORACLE MANIPULATION SUSPECTED: High deviation between oracle sources. "
        ...
        if twap_price > 0:
            for source, price in current_prices.items():
                ...
                if deviation_from_twap > self.deviation_threshold_percentage:
                    print(f"!!! ORACLE MANIPULATION SUSPECTED: Price from {source} ({price:.4f}) deviates significantly from TWAP ({twap_price:.4f}). "
...
if __name__ == "__main__":
    ...
    manipulation_cb = CircuitBreaker(name="OracleManipulationCB", failure_threshold=1, recovery_timeout_seconds=60)

    # Setup Detector
    detector = OracleManipulationDetector(twap_oracle, manipulation_cb, deviation_threshold_percentage=2.0) # 2% threshold

    print("--- Scenario 1: Normal Operation ---")
    current_prices_normal = {
        "Chainlink": 101.5,
        "Uniswap": 101.6,
        "Band": 101.4
    }
