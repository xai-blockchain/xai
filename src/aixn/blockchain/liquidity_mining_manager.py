...
class LiquidityMiningManager:
    ...
    def distribute_rewards(self, amount: float) -> float:
        ...
        if amount_to_distribute <= 0:
            print(
                f"Cannot distribute {amount:.2f} rewards. Daily cap of {self.daily_reward_cap:.2f} reached."
            )
            return 0.0

        self.rewards_distributed_today += amount_to_distribute
        print(
            f"Distributed {amount_to_distribute:.2f} rewards. Total distributed today: {self.rewards_distributed_today:.2f}"
        )
