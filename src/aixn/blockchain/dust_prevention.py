...
class DustPreventionManager:
    ...
    def process_incoming_transaction(self, to_address: str, amount: float, from_address: str = "0xSender"):
        """
        ...
        if self.is_dust_amount(amount):
            print(f"!!! DUST ATTACK DETECTED !!! Incoming transaction of {amount:.8f} {self.asset_symbol} to {to_address} from {from_address} is below dust threshold.")
            ...
            self.wallet_balances[to_address] = self.wallet_balances.get(to_address, 0.0) + amount
            print(f"Processed incoming transaction: {amount:.8f} {self.asset_symbol} to {to_address}. New balance: {self.wallet_balances[to_address]:.8f}")
    ...
    def consolidate_dust(self, main_address: str, addresses_to_consolidate: List[str]):
        ...
        for address in addresses_to_consolidate:
            ...
            if self.is_dust_amount(balance) and balance > 0:
                ...
                print(f"Address {address} has {balance:.8f} {self.asset_symbol}, which is above dust threshold. Not consolidating.")
        
        if total_consolidated_amount > 0:
            self.wallet_balances[main_address] = self.wallet_balances.get(main_address, 0.0) + total_consolidated_amount
            print(f"Total {total_consolidated_amount:.8f} {self.asset_symbol} consolidated to {main_address}. New balance: {self.wallet_balances[main_address]:.8f}")
        else:
            print("No dust amounts found for consolidation.")

# Example Usage (for testing purposes)
if __name__ == "__main__":
    dust_manager = DustPreventionManager(asset_symbol="BTC", dust_threshold_amount=0.00001) # 1000 satoshis
