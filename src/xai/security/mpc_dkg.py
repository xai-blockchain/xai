...
if __name__ == "__main__":
    print("=== MPC Distributed Key Generation Test ===\n")

    # Test Shamir's Secret Sharing
    print("1. Testing Shamir's Secret Sharing")
    ...
    print(f"   Threshold: {threshold}/{num_shares}")

    shares = sss.generate_shares(secret, threshold, num_shares)
    print(f"   Generated {len(shares)} shares")

    # Verify shares
    all_valid = all(sss.verify_share(share) for share in shares)
    ...
    # Test MPC-DKG
    print("2. Testing MPC Distributed Key Generation")
    mpc_dkg = MPCDistributedKeyGeneration()

    num_participants = 5
    threshold = 3

    results, common_pubkey = mpc_dkg.generate_distributed_keys(num_participants, threshold)
    print(f"   Generated keys for {num_participants} participants")
    print(f"   Threshold: {threshold}")
    print(f"   Common public key length: {len(common_pubkey)} bytes")

    # Verify all results
    all_verified = all(mpc_dkg.verify_dkg_result(r) for r in results)
    print(f"   All DKG results verified: {all_verified}")

    # Reconstruct private key from threshold shares
    print(f"\n   Reconstructing private key from {threshold} shares...")
    reconstructed_privkey = mpc_dkg.reconstruct_private_key(results[:threshold])
    print(f"   Reconstructed private key length: {len(reconstructed_privkey)} bytes")
    print(f"   Success: Private key reconstructed from distributed shares")
