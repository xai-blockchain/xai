...
class MockMPCDKG(MPCDKGInterface):
            ...
            participant_data.append(
                {
                    ...
                    "share": shares[i],
                    "common_public_key": str(
                        mock_master_public_key
                    ),  # All participants know the common public key
                }
            )
    ...
    def reconstruct_public_key(self, participant_shares: List[Dict]) -> str:
        ...
        if str(reconstructed_sum) == expected_public_key:
            print(
                f"MockMPCDKG: Successfully reconstructed public key (sum of shares: {reconstructed_sum})."
            )
            return expected_public_key
        else:
            raise ValueError(
                f"MockMPCDKG: Public key reconstruction failed. Sum of shares: {reconstructed_sum}, Expected: {expected_public_key}"
            )
