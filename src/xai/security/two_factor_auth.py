...
if __name__ == "__main__":
    print("=== Two-Factor Authentication Test ===\n")

    # Initialize 2FA manager
    tfa_manager = TwoFactorAuthManager(issuer_name="XAI Blockchain")

    # Set up 2FA for a user
    print("1. Setting up 2FA for user")
    setup = tfa_manager.setup_2fa(
        user_id="user123",
        user_email="user@example.com"
    )

    print(f"   Secret: {setup.secret}")
    print(f"   Provisioning URI: {setup.provisioning_uri[:60]}...")
    print(f"   Backup codes ({len(setup.backup_codes)}):")
    for i, code in enumerate(setup.backup_codes[:3], 1):
        print(f"     {i}. {code}")
    print(f"   ... (showing first 3 of {len(setup.backup_codes)})\n")

    # Generate and verify TOTP
    print("2. Testing TOTP generation and verification")
    current_code = tfa_manager.generate_totp(setup.secret)
    print(f"   Current TOTP: {current_code}")

    is_valid = tfa_manager.verify_totp(setup.secret, current_code)
    print(f"   Verification: {is_valid}")

    # Test invalid code
    invalid_code = "000000"
    is_invalid = tfa_manager.verify_totp(setup.secret, invalid_code)
    print(f"   Invalid code verification: {is_invalid}\n")

    # Test backup codes
    print("3. Testing backup code verification")
    hashed_backups = tfa_manager.hash_backup_codes(setup.backup_codes)
    print(f"   Generated {len(hashed_backups)} hashed backup codes")

    # Verify a backup code
    test_backup = setup.backup_codes[0]
    is_valid_backup, updated_codes = tfa_manager.verify_backup_code(
        test_backup,
        hashed_backups
    )
    print(f"   Backup code '{test_backup}' valid: {is_valid_backup}")
    if updated_codes:
        print(f"   Remaining backup codes: {len(updated_codes)}")

    # Try to reuse the same backup code
    is_reused, _ = tfa_manager.verify_backup_code(test_backup, updated_codes)
    print(f"   Reusing same backup code: {is_reused} (should be False)\n")

    # Generate recovery codes
    print("4. Generating recovery codes")
    recovery_codes = tfa_manager.generate_recovery_codes(num_codes=5)
