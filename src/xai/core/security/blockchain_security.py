...
class ReorganizationProtection:
    ...
    def create_checkpoint(self, block_index: int, block_hash: str) -> None:
        """
        Alias for add_checkpoint() for backwards compatibility

        Args:
            block_index: Block number
            block_hash: Block hash
        """
        self.add_checkpoint(block_index, block_hash)

    def has_checkpoint(self, block_index: int) -> bool:
        """
        Alias for is_checkpoint_block() for backwards compatibility
        ...
        """
        return self.is_checkpoint_block(block_index)

    def is_protected(self, block_index: int, block_hash: str) -> bool:
        """
        Check if block is protected by matching checkpoint
        ...
        Returns:
            bool: True if block matches checkpoint
        """
        checkpoint_hash = self.get_checkpoint(block_index)
...
class SupplyValidator:
    ...
    def validate_block_reward(self, reward: float, total_fees: float) -> tuple[bool, str]:
        ...
        Note: This validates the reward is positive. Actual reward calculation
        based on halving schedule should be done in the blockchain logic.
        """
        if reward <= 0:
            return False, "Block reward must be positive"

        # Validate reward is not excessively large (e.g., attempt to mint too many coins)
        # Using a reasonable upper bound of 100 AXN per block
        if reward > 100.0:
            return False, "Block reward exceeds maximum allowed (100 AXN)"
...
class OverflowProtection:
    ...
    def validate_amount(self, amount: float) -> tuple[bool, str]:
        ...
        from xai.core.consensus.validation import validate_amount as core_validate_amount
