...
class AdvancedRateLimiter:
    ...
    def __init__(self):
        ...
        self.endpoint_configs: Dict[str, RateLimitConfig] = {
            ...
            "/api/login": RateLimitConfig(5, 300, per_ip=True),
            "/api/register": RateLimitConfig(3, 3600, per_ip=True),
            "/api/password-reset": RateLimitConfig(3, 3600, per_ip=True),

            # Wallet operations
            "/api/wallet/send": RateLimitConfig(50, 60, per_user=True),
            "/api/wallet/create": RateLimitConfig(5, 3600, per_ip=True),

            # Mining operations
            "/api/mining/start": RateLimitConfig(10, 60, per_user=True),
            "/api/mining/submit": RateLimitConfig(1000, 60, per_user=True),

            # Public endpoints - more lenient
            "/api/blocks": RateLimitConfig(500, 60, per_ip=True),
            "/api/transactions": RateLimitConfig(500, 60, per_ip=True),
            "/api/balance": RateLimitConfig(200, 60, per_ip=True),

            # Admin/governance - restricted
            "/api/governance/vote": RateLimitConfig(10, 60, per_user=True),
            "/api/admin/config": RateLimitConfig(5, 60, per_user=True),
        }
