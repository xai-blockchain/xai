...
class BlockPruningManager:
    ...
    def _archive_block(self, block: Block) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to archive block %d: %s",
                block.index,
                str(e),
                extra={"event": "pruning.archive_error", "height": block.index, "error": str(e)}
            )
            return False
    ...
    def _prune_block(self, block: Block) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to prune block %d: %s",
                block.index,
                str(e),
                extra={"event": "pruning.prune_error", "height": block.index, "error": str(e)}
            )
            return False
    ...
    def restore_block(self, height: int) -> Optional[Block]:
        ...
        try:
            ...
            # Reconstruct block (would need to use actual Block class)
            # This is a placeholder - actual implementation depends on Block class
            logger.info(
            ...
            return block_dict  # type: ignore

        except Exception as e:
            logger.error(
                "Failed to restore block %d: %s",
                height,
                str(e),
                extra={"event": "pruning.restore_error", "height": height, "error": str(e)}
            )
            return None
