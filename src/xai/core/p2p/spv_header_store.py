...
class Header:
    ...
    def work(self) -> int:
        """
        Calculate proof-of-work contribution using Bitcoin-style formula.

        Work is calculated as: 2^256 / (target + 1)

        This means lower targets (harder blocks) contribute more work.
        The cumulative work metric ensures the heaviest chain wins during reorgs.

        Returns:
            int: Work value for this block header
        """
        target = self.compact_to_target(self.bits)
        if target <= 0:
            return 1  # Minimum work for invalid targets
        # Bitcoin-style work: 2^256 / (target + 1)
        # Using bit shifts for efficiency with large numbers
        two_256 = 1 << 256
        return two_256 // (target + 1)
