...
class CheckpointSyncManager:
    ...
    def _download_chunk(self, snapshot_id: str, chunk_index: int) -> 'SyncChunk' | None:
        ...
        # P2P chunk download from connected peers
        ...
        # Query connected peers for chunk availability
        peers = self._get_connected_peers()
        if not peers:
            logger.debug(
                "No connected peers for chunk download",
                extra={"snapshot_id": snapshot_id, "chunk_index": chunk_index}
            )
            return None

        timeout = 30.0  # seconds per peer attempt
        for peer in peers[:3]:  # Try up to 3 peers
            try:
                chunk_data = self._request_chunk_from_peer(
                    peer, snapshot_id, chunk_index, timeout
                )
                if chunk_data:
                    # Verify chunk hash matches expected (if metadata available)
                    if self.chunked_service:
                        metadata = self.chunked_service.get_snapshot_metadata(snapshot_id)
                        if metadata and chunk_index < len(metadata.chunk_hashes):
                            expected_hash = metadata.chunk_hashes[chunk_index]
                            actual_hash = hashlib.sha256(chunk_data.data).hexdigest()
                            if actual_hash != expected_hash:
                                logger.warning(
                                    "Chunk hash mismatch from peer",
                                    extra={
                                        "snapshot_id": snapshot_id,
                                        "chunk_index": chunk_index,
                    ...
                    logger.debug(
                        "Downloaded chunk from peer",
                        extra={
                            "snapshot_id": snapshot_id,
                            "chunk_index": chunk_index,
                            "size": chunk_data.size_bytes,
                        }
                    )
                    return chunk_data
                ...
                logger.debug(
                    "Failed to download chunk from peer: %s",
                    str(e),
                    extra={
                        "snapshot_id": snapshot_id,
                        "chunk_index": chunk_index,
    ...
    def _get_connected_peers(self) -> list[Any]:
        """Get list of connected peers from P2P manager."""
        if not self.p2p_manager:
            return []

        # Try different peer accessor patterns
        if hasattr(self.p2p_manager, 'get_connected_peers'):
            return self.p2p_manager.get_connected_peers() or []
        if hasattr(self.p2p_manager, 'connected_peers'):
            peers = self.p2p_manager.connected_peers
            return list(peers.values()) if isinstance(peers, dict) else list(peers)
        if hasattr(self.p2p_manager, 'peers'):
            peers = self.p2p_manager.peers
            return list(peers.values()) if isinstance(peers, dict) else list(peers)
        return []

    def _request_chunk_from_peer(
        self,
        peer: Any,
        snapshot_id: str,
        chunk_index: int,
        ...
        Args:
            ...
            chunk_index: Index of chunk to request
            timeout: Request timeout in seconds

        Returns:
            SyncChunk if successful, None otherwise
        """
        request = {
            "type": "chunk_request",
            "snapshot_id": snapshot_id,
        ...
        elif hasattr(peer, 'request'):
            response = peer.request(request, timeout=timeout)

        if not response or not isinstance(response, dict):
            return None

        # Parse response into SyncChunk
        if response.get("type") != "chunk_response":
            return None
