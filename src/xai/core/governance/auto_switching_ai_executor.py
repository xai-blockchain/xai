...
class AutoSwitchingAIExecutor:
    ...
    def __init__(self, pool_manager, key_manager):
        ...
        if NEW_PROVIDERS_AVAILABLE:
            ...
            print(
                "✅ Initialized 6 additional AI providers (Perplexity, Groq, xAI, Together, Fireworks, DeepSeek)"
            )
        else:
    ...
    def execute_long_task_with_auto_switch(
        ...
        context = ConversationContext(messages=[], system_prompt=None)
    ...
    def _execute_with_streaming_and_switching(
        ...
        if provider.value == "anthropic":
            ...
            stream = client.chat.completions.create(
                model="gpt-4-turbo", max_tokens=safe_limit, messages=context.messages, stream=True
            )
            ...
            result = self.perplexity.call_with_limit(
                api_key=current_api_key, task=task_text, max_tokens=safe_limit
            )
        ...
        return {
            ...
            "swap_events": [
                ...
                for e in self.swap_events
                if any(k == e.old_key_id or k == e.new_key_id for k in task_state["keys_used"])
            ],
        }
    ...
    def _execute_with_switching(
        ...
        while total_tokens_used < max_total_tokens:
            ...
            this_chunk_size = min(
                chunk_size, key_remaining - self.buffer_tokens, max_total_tokens - total_tokens_used
            )
            ...
            if provider.value == "anthropic":
                ...
                response = client.messages.create(
                    model=context.model, max_tokens=this_chunk_size, messages=context.messages
                )
                ...
                response = client.chat.completions.create(
                    model="gpt-4-turbo", max_tokens=this_chunk_size, messages=context.messages
                )
                ...
                task_text = context.messages[-1]["content"] if context.messages else ""
                result = self.perplexity.call_with_limit(
                    api_key=current_api_key, task=task_text, max_tokens=this_chunk_size
                )
    ...
    def _switch_to_next_key(
        ...
        print(
            f"   Available: {new_key_metadata['donated_tokens'] - new_key_metadata['used_tokens']:,} tokens"
        )
...
if __name__ == "__main__":
    ...
    from xai.core.security.ai_pool_with_strict_limits import StrictAIPoolManager
    from xai.core.security.secure_api_key_manager import AIProvider, SecureAPIKeyManager
