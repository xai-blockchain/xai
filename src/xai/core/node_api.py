...
class NodeAPIRoutes:
    ...
    def _setup_blockchain_routes(self) -> None:
        ...
        @validate_request(self.request_validator, PeerBlockInput)
        def receive_block() -> Tuple[Dict[str, Any], int]:
            ...
            model: Optional[PeerBlockInput] = getattr(request, "validated_model", None)
            if model is None:
                return self._error_response(
            ...
            try:
                ...
                block = Blockchain.deserialize_block(model.dict())
    ...
    def _setup_transaction_routes(self) -> None:
        ...
        @validate_request(self.request_validator, PeerTransactionInput)
        def receive_transaction() -> Tuple[Dict[str, Any], int]:
            """Receive a broadcasted transaction from a peer node."""
            auth_error = self._require_api_auth()
            if auth_error:
                return auth_error
            model: Optional[PeerTransactionInput] = getattr(request, "validated_model", None)
            if model is None:
                return self._error_response(
                    "Invalid transaction payload",
                    status=400,
            ...
            try:
                ...
                tx.timestamp = model.timestamp or time.time()
                tx.signature = model.signature
                tx.txid = model.txid or tx.calculate_hash()
                if model.metadata:
                    tx.metadata = model.metadata
            except Exception as exc:
                return self._error_response(
                    ...
                    context={"sender": model.sender, "error": str(exc)},
                )
            ...
            return self._error_response(
                ...
                status=400,
                code="transaction_rejected",
                context={"sender": model.sender, "txid": tx.txid},
            )
    ...
    def _setup_algo_routes(self) -> None:
        ...
        def estimate_fee() -> Tuple[Dict[str, Any], int]:
            ...
            if not (ALGO_FEATURES_ENABLED and fee_optimizer):
        ...
        @validate_request(self.request_validator, FraudCheckInput)
        def check_fraud() -> Tuple[Dict[str, Any], int]:
            ...
            if not (ALGO_FEATURES_ENABLED and fraud_detector):
                return jsonify({"error": "Algorithmic features not available"}), 503

            model: Optional[FraudCheckInput] = getattr(request, "validated_model", None)
            if model is None:
                return self._error_response(
            ...
            analysis = fraud_detector.analyze_transaction(model.payload)
    ...
    def _setup_exchange_balance_routes(self) -> None:
        ...
        def withdraw_funds() -> Tuple[Dict[str, Any], int]:
            ...
            try:
                model: Optional[ExchangeTransferInput] = getattr(request, "validated_model", None)
                if model is None or not model.destination:
                    return self._error_response(
        ...
        def buy_with_card() -> Tuple[Dict[str, Any], int]:
            ...
            try:
                ...
                payment_result = self.node.payment_processor.process_card_payment(
                    ...
                    card_token=model.card_token or request.json.get("card_token", "tok_test"),
                    email=model.email,
                )
