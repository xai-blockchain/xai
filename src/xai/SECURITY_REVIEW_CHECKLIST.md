...
- [x] Proper prefix (XAI for mainnet, TXAI for testnet)
...
- [x] Peer reputation system enforced via `P2PSecurityManager`/`PeerReputation` (see `src/xai/core/p2p_security.py`).
- [x] Connection limits enforced through `NodeConnectionManager` inbound/outbound guards.
- [x] DDoS protection handled with rate limiting + packet inspection in `MessageRateLimiter`.
- [x] Peer messages validated for size/type using `P2PSecurityManager.validate_message`.
...
- [x] Optional API authentication now available via configurable API keys and `/admin/api-keys` rotation endpoints.
- [x] All API payloads flow through request validation middleware + `validate_api_request` for sanitization and size checks.
- [x] Injection attacks mitigated by `SecurityValidator` sanitization and strict Pydantic schemas.
...
- [x] Amount validation (prevent negative/overflow via `SecurityValidator` + `NodeTransactionInput`).
- [x] Fee validation (prevent excessive fees via validator + Pydantic guards).
- [x] Address format validation (XAI/TXAI prefix enforced by `SecurityValidator`).

### ⚠️ API Input
- [x] Validate all JSON inputs (global RequestValidator + endpoint-specific Pydantic schemas).
- [x] Sanitize user inputs (SecurityValidator truncation + sanitized error responses/logging).
- [ ] **TODO:** Prevent SQL injection (if database added)
- [ ] **TODO:** Prevent XSS (if web interface added)
- [x] Limit input sizes (RequestValidator enforces `Config.API_MAX_JSON_BYTES`).
...
- [x] Never expose internal errors to users (Node API now returns sanitized helper responses only).
- [x] Log errors securely (security logger masks addresses/IP via `_log_event`).
- [x] Fail securely (don't expose sensitive data) with centralized `_error_response` + `ErrorHandlerRegistry` integration.
