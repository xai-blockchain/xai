...
class EclipseProtector:
    ...
    def connect_peer(self, ip_address: str) -> str:
        ...
        if self.connections_by_ip[ip_address] >= self.max_connections_per_ip:
            raise ValueError(
                f"Connection from IP {ip_address} rejected: Exceeds max connections per IP ({self.max_connections_per_ip})."
            )
    ...
    def check_for_eclipse_risk(self) -> bool:
        ...
        if unique_ips < self.min_diverse_peers:
            print(
                f"!!! ECLIPSE RISK ALERT !!! Only {unique_ips} unique IPs connected, less than minimum diverse peers ({self.min_diverse_peers})."
            )
            return True
