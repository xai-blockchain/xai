...
class FeeAdjuster:
    def __init__(
        ...
        max_fee: float = 0.1,
        congestion_factor: float = 0.01,
        history_window_blocks: int = 10,
    ):
    ...
    def update_network_metrics(self, pending_transactions: int, block_fullness: float):
        ...
        print(
            f"Network metrics updated: Pending TXs={pending_transactions}, Block Fullness={block_fullness:.2f}"
        )
    ...
    def get_suggested_fee(self) -> float:
        ...
        avg_pending_tx = sum([m[0] for m in self.network_metrics_history]) / len(
            self.network_metrics_history
        )
        avg_block_fullness = sum([m[1] for m in self.network_metrics_history]) / len(
            self.network_metrics_history
        )
        ...
        congestion_score = (
            avg_pending_tx / 1000.0
        ) + avg_block_fullness  # Normalize pending_tx for example
