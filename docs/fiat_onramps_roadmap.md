...
2. **KYC & AML controls**  
   ...
   - Feed every fiat transaction into `TransactionRiskScore` (see `xai/core/aml_compliance.py`) so the risk level, flagged reasons, and required approvals are captured before the trade is settled.  
...
- A new endpoint, `POST /governance/fiat-unlock/vote` (see `xai/core/api_extensions.py`), accepts `governance_address`, `support`, and optional `reason`. Submitters must use their XAI address (`XAI...`) to vote.  
...
- The fee treasury now emits Prometheus metrics (`xai_trade_fee_collected_total`, `xai_fee_treasury_balance`, etc.) in `xai/core/treasury_metrics.py`, so Grafana dashboards can display how much XAI accrues from wallet trades.  
...
- `TransactionRiskScore` assigns 0-100 scores based on large amounts, structuring, rapid succession, rounding patterns, and velocity spikes (`xai/core/aml_compliance.py`).  
