...
### DEFECT-002: Bootstrap Script Uses Placeholder Git Repository URL
...
**Root Cause:**
The EC2 user_data bootstrap script attempted to clone blockchain code from a placeholder GitHub URL (`https://github.com/YOUR_ORG/xai-blockchain.git`) that doesn't exist. This caused the entire bootstrap process to fail silently. The blockchain application never got installed, so no process listened on port 5000, causing health checks to fail continuously.

**Location:**
```
File: deploy/aws/terraform/user_data.sh
Line: 52 (original)
Command: git clone https://github.com/YOUR_ORG/xai-blockchain.git .
```
...
```bash
# Replaced git clone with S3 download (lines 52-56):
...
1. **Template Code Not Production-Ready**
   - Placeholder GitHub URL left in user_data
