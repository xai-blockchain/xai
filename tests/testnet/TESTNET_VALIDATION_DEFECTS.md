...
### DEFECT-002: User Data Script Uses Placeholder Git Repository URL
...
**Description:**
The EC2 user_data script (`deploy/aws/terraform/user_data.sh` line 52) attempts to clone blockchain code from a placeholder GitHub URL that doesn't exist. This causes the bootstrap process to fail, and the blockchain application never starts.
...
4. **GitHub Repo**: Create and use actual GitHub repository
...
2. 🔄 DEFECT-002: In Progress - Need to deploy code to instances
   ...
   - Update user_data.sh to download from S3 instead of git clone
