"""
Unit tests for JWT Auth Manager blacklist automatic cleanup functionality.

Tests the background thread cleanup, thread safety, and configuration options
for the jwt_auth_manager.py implementation.
"""
...
class TestJWTAuthManagerCleanup:
    ...
    def test_manual_cleanup_removes_expired_tokens(self):
        ...
        access_token = mgr.generate_token("user123", "testuser", UserRole.USER)
        ...
        removed = mgr.cleanup_revoked_tokens()
    ...
    def test_cleanup_handles_mixed_tokens(self):
        ...
        short_token = mgr_short.generate_token("user1", "testuser1", UserRole.USER)
        mgr_short.revoke_token(short_token)

        # Generate long-lived token
        long_token = mgr_long.generate_token("user2", "testuser2", UserRole.USER)
        mgr_long.revoke_token(long_token)
        ...
        # Add both tokens to combined manager's revocation list
        # Note: This is a bit hacky but needed for testing
        mgr_combined.revoke_token(short_token)
        mgr_combined.revoke_token(long_token)

        initial_size = mgr_combined.get_blacklist_size()
        ...
        # Trigger cleanup
        removed = mgr_combined.cleanup_revoked_tokens()

        # Should remove at least 1 expired token
        assert removed >= 1
        assert mgr_combined.get_blacklist_size() < initial_size
    ...
    def test_thread_safety_concurrent_revocations(self):
        ...
        tokens = []
        for i in range(10):
            access_token = mgr.generate_token(f"user{i}", f"testuser{i}", UserRole.USER)
    ...
    def test_thread_safety_read_write_concurrent(self):
        ...
        def writer_worker():
            ...
            while not stop_event.is_set():
                try:
                    token = mgr.generate_token(f"writer_user_{counter}", f"writer{counter}", UserRole.USER)
    ...
    def test_get_blacklist_size_thread_safe(self):
        ...
        # Add initial tokens
        for i in range(10):
            token = mgr.generate_token(f"user{i}", f"testuser{i}", UserRole.USER)
            mgr.revoke_token(token)
        ...
        def token_adder():
            ...
            counter = 0
            while not stop_event.is_set():
                token = mgr.generate_token(f"adder_user_{counter}", f"adder{counter}", UserRole.USER)
                mgr.revoke_token(token)
                counter += 1
                time.sleep(0.01)
