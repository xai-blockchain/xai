...
from xai.blockchain.vesting_manager import VestingManager, VestingScheduleData
...
def test_vesting_schedule_claim_flow():
    ...
    schedule_data = VestingScheduleData("0xuser", 1000.0, start, end, cliff)
    schedule_id = manager.create_vesting_schedule(schedule_data)
...
def test_vesting_schedule_validation_errors():
    ...
    with pytest.raises(ValueError):
        manager.create_vesting_schedule(VestingScheduleData("", 100.0, now, now + 10, 0))
    with pytest.raises(ValueError):
        manager.create_vesting_schedule(VestingScheduleData("0xuser", -5, now, now + 10, 0))
    with pytest.raises(ValueError):
        manager.create_vesting_schedule(VestingScheduleData("0xuser", 100.0, now + 20, now + 10, 0))
    with pytest.raises(ValueError):
        manager.get_vested_amount("unknown")
    schedule_id = manager.create_vesting_schedule(VestingScheduleData("0xuser", 50.0, now, now + 10, 0))
