...
VALID_TEST_ADDRESS = "XAI" + "1" * 40  # Valid hex address: XAI followed by 40 hex digits
VALID_RECIPIENT_ADDRESS = "XAI" + "2" * 40  # Valid hex address: XAI followed by 40 hex digits


@pytest.fixture
def app_client():
    node = BlockchainNode(miner_address="XAI_TEST_MINER")
    node.personal_ai = PersonalAIAssistant(node.blockchain, safety_controls=None)
...
def test_personal_ai_contract_create(app_client):
    ...
    response = app_client.post(
        "/personal-ai/smart-contract/create", json=payload, headers=_headers()
    )
    assert response.status_code == 200
    data = response.get_json()
    assert data["success"]
