...
def mock_node(temp_blockchain_dir):
    ...
    # Provision signing identity for mining/signature validation
    identity_wallet = Wallet()
    node.blockchain.node_identity = {
    ...
    # Realistic peer manager for signed P2P HTTP ingress
    ...
    # Override gamification managers with test doubles to allow return_value patching
    ...
    node.blockchain.treasure_manager.claim_treasure = Mock(return_value=(True, 0.0))
    node.blockchain.treasure_manager.get_treasure_details = Mock(return_value={"id": "treasure123"})
    node.blockchain.streak_tracker = Mock()
    node.blockchain.streak_tracker.get_leaderboard = Mock(return_value=[])
    node.blockchain.streak_tracker.get_miner_stats = Mock(return_value={"current_streak": 0})
    node.blockchain.streak_tracker.apply_streak_bonus = Mock(
        side_effect=lambda miner, reward: (float(reward), 0.0)
    )
...
class TestTransactionEndpoints:
    ...
    def test_get_pending_transactions_with_txs(self, flask_app, mock_node):
        ...
        wallet = Wallet()
        mock_node.blockchain.utxo_manager.add_utxo(wallet.address, "funding", 0, 100.0, f"P2PKH {wallet.address}")
        tx = Transaction(
            wallet.address,
            "XAI" + "B" * 40,
            10.0,
            fee=0.01,
            nonce=mock_node.blockchain.nonce_tracker.get_next_nonce(wallet.address),
            inputs=[{"txid": "funding", "vout": 0, "amount": 100.0}],
            outputs=[
                {"address": "XAI" + "B" * 40, "amount": 10.0},
                {"address": wallet.address, "amount": 89.99},
            ],
        )
    ...
    def test_get_transaction_confirmed(self, flask_app, mock_node):
        ...
        wallet = Wallet()
        coinbase_addr = wallet.address
        mock_node.blockchain.utxo_manager.add_utxo(wallet.address, "funding", 0, 50.0, f"P2PKH {wallet.address}")
        tx = Transaction(
            wallet.address,
            "XAI" + "C" * 40,
            5.0,
            fee=0.01,
            nonce=mock_node.blockchain.nonce_tracker.get_next_nonce(wallet.address),
            inputs=[{"txid": "funding", "vout": 0, "amount": 50.0}],
            outputs=[
                {"address": "XAI" + "C" * 40, "amount": 5.0},
                {"address": wallet.address, "amount": 44.99},
            ],
        )
        tx.public_key = wallet.public_key
        tx.sign_transaction(wallet.private_key)
        mock_node.blockchain.pending_transactions.append(tx)
        block = mock_node.blockchain.mine_pending_transactions(coinbase_addr, node_identity=mock_node.blockchain.node_identity)
...
class TestMiningEndpoints:
    ...
    def test_mine_success(self, flask_app, mock_node):
        ...
        tx = Transaction(
            ...
            inputs=[{"txid": "funding", "vout": 0, "amount": 50.0}],
            outputs=[
                {"address": "XAI" + "D" * 40, "amount": 10.0},
                {"address": wallet.address, "amount": 39.99},
            ],
        )
        tx.public_key = wallet.public_key
        tx.sign_transaction(wallet.private_key)
...
class TestAlgorithmicEndpoints:
    ...
    def test_check_fraud_success(self, flask_app, mock_node):
        ...
        with patch('xai.core.node_utils.ALGO_FEATURES_ENABLED', True):
            tx_data = {"payload": {"sender": "addr1", "recipient": "addr2", "amount": 100}}
...
class TestRecoveryEndpoints:
    ...
    def test_execute_recovery_success(self, flask_app, mock_node):
        ...
        execute_data = {"request_id": "req123", "executor_address": "owner1"}
