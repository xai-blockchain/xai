...
class TestSybilAttack:
    ...
    def test_stake_based_voting_prevents_sybil(self, tmp_path):
        ...
        # Mine multiple blocks - rewards need confirmations
        for _ in range(5):
...
class TestRaceAttack:
    ...
    def test_transaction_confirmation_requirement(self, tmp_path):
        """Test transactions require confirmations"""
        bc = Blockchain(data_dir=str(tmp_path))
        sender = Wallet()
        recipient = Wallet()

        # Give sender balance - mine multiple blocks for rewards to be spendable
        for _ in range(5):
            bc.mine_pending_transactions(sender.address)

        # Create transaction using blockchain's method (handles UTXO properly)
        tx = bc.create_transaction(
            sender.address, recipient.address, 5.0, 0.24, sender.private_key, sender.public_key
        )

        # Add transaction if created successfully
        if tx:
            bc.add_transaction(tx)
...
class TestInflationAttack:
    ...
    def test_block_reward_calculation_safety(self, tmp_path):
        """Test block reward calculation is safe"""
        bc = Blockchain(data_dir=str(tmp_path))

        # Test various block heights
        for height in [0, 100000, 262800, 525600]:  # Test different halving periods
