...
class TestCascadingFailures:
    ...
    def test_multiple_node_failures(self, tmp_path):
        ...
        # Node 2 continues
        block = nodes[1].mine_pending_transactions(Wallet().address)
    ...
    def test_consensus_failure_recovery(self, tmp_path):
        ...
        # Divergence
        for _ in range(3):
            block1 = bc1.mine_pending_transactions(Wallet().address)

        for _ in range(2):
            block2 = bc2.mine_pending_transactions(Wallet().address)

        # bc1 is longer, bc2 should reorganize
        for block in bc1.chain[len(bc2.chain):]:
            bc2.add_block(block)
