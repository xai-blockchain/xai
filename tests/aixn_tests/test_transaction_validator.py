...
def valid_transaction():
    wallet1 = Wallet()
    wallet2 = Wallet()
    tx = Transaction(wallet1.address, wallet2.address, 10.0, 0.24, public_key=wallet1.public_key, nonce=0)
...
class TestTransactionValidator:
    ...
    def test_missing_required_fields(self, validator):
        ...
        incomplete_tx = Transaction("sender", "recipient", 10.0) # Missing fee, public_key, nonce etc.
        incomplete_tx.txid = incomplete_tx.calculate_hash() # Manually set txid to avoid hash mismatch first
    ...
    def test_invalid_nonce(self, validator, valid_transaction, mock_nonce_tracker):
        ...
        mock_nonce_tracker.get_next_nonce.return_value = 1 # Simulate expected nonce is 1, but tx.nonce is 0
    ...
    def test_time_capsule_lock_validation(self, validator):
        ...
        wallet1 = Wallet()
        wallet2 = Wallet()
        tx = Transaction(wallet1.address, wallet2.address, 10.0, 0.24, public_key=wallet1.public_key, tx_type="time_capsule_lock", nonce=0)
        tx.sign_transaction(wallet1.private_key)
        ...
        with pytest.raises(ValidationError, match="Time capsule unlock_time must be in the future."):
            validator.validate_transaction(tx)
        validator.logger.warn.assert_called_once()
    ...
    def test_governance_vote_validation(self, validator):
        ...
        wallet1 = Wallet()
        wallet2 = Wallet()
        tx = Transaction(wallet1.address, wallet2.address, 0.0, 0.0, public_key=wallet1.public_key, tx_type="governance_vote", nonce=0)
        tx.sign_transaction(wallet1.private_key)
    ...
    def test_get_transaction_validator_singleton(self, mock_blockchain, mock_nonce_tracker, mock_logger):
