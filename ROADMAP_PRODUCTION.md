...
- [ ] Split `APIRoutes.__init__` (2,183 lines) - extract route definitions into separate modules by category. **Progress:** Created `api_routes/admin.py` (7 routes, 265 lines) and `api_routes/crypto_deposits.py` (5 routes, 182 lines). Both modules have complete docstrings and production-grade error handling. Updated `api_routes/__init__.py` with exports. **Remaining:** Eliminate duplication by removing `_setup_admin_routes()` (lines 2049-2228) and `_setup_crypto_deposit_routes()` (lines 1959-2046) from node_api.py, then update setup_routes() to call the new registration functions. Estimated reduction: ~350 lines from node_api.py.
...
- [ ] Add docstrings to 77 public APIs missing documentation across 16 files. **ACTUAL PROGRESS:** Only 12/77 route function docstrings actually applied (admin.py: 7, crypto_deposits.py: 5). Existing files (peer.py, mining.py, algo.py, wallet.py) still missing route-level docstrings - only module-level register() functions documented. Agent provided complete docstring templates for 25 methods, but NOT yet applied to code. **Remaining work:** Apply 65 docstrings to existing files. **Priority 1**: api_routes/exchange.py (15), api_routes/recovery.py (10), api_routes/gamification.py (12), api_routes/mining_bonus.py (9), api_routes/contracts.py (7), api_routes/wallet.py (4). **Priority 2**: node_api.py (7), peer.py (2), mining.py (2), algo.py (2).
