...
import os
import time
import secrets  # SECURITY: Use secrets module for cryptographic operations
import hashlib

from src.xai.core.blockchain import Blockchain, Transaction, Block
from src.xai.core.wallet import Wallet

# SECURITY NOTE: Using secrets.SystemRandom() instead of random module
# for all cryptographic operations to ensure cryptographically secure randomness
...
def create_weighted_miner_pool(wallets):
    ...
    for wallet in wallets:
        ...
        weight = secure_random.randint(1, 5)
        for _ in range(weight):
            weighted_pool.append(wallet)

    secure_random.shuffle(weighted_pool)
...
def select_miner(weighted_pool, block_index, used_indices):
    ...
    attempts = 0
    while attempts < 50:
        idx = secure_random.randint(0, len(weighted_pool) - 1)
...
def create_inter_wallet_transaction(wallets, current_timestamp):
    ...
    sender = secrets.choice(wallets)
    recipient = secrets.choice([w for w in wallets if w["address"] != sender["address"]])

    # Random amount (0.1 to 100 XAI)
    amount = round(secure_random.uniform(0.1, 100), 2)
...
def premine_blockchain(blocks_to_mine=BLOCKS_TO_MINE):
    ...
    for block_num in range(1, blocks_to_mine + 1):
        ...
        time_delta = BASE_BLOCK_TIME + secure_random.randint(-TIME_VARIANCE, TIME_VARIANCE)
        current_timestamp += time_delta

        # Optionally add inter-wallet transaction
        if secure_random.random() < TX_PROBABILITY and len(blockchain.pending_transactions) == 0:
