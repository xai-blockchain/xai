#!/usr/bin/env python3
"""
Publish withdrawal threshold summaries to GitHub issues (or run in dry-run mode).
"""
...
def _detect_repo(explicit: Optional[str]) -> str:
    ...
    if env_repo:
        return env_repo
    raise SystemExit("GitHub repository not provided. Use --github-repo or set GITHUB_REPOSITORY.")


def _detect_token(explicit: Optional[str]) -> str:
    token = explicit or os.environ.get("GITHUB_TOKEN")
    if not token:
        raise SystemExit("GitHub token not provided. Use --token or set GITHUB_TOKEN.")
    return token
...
def _post_github_comment(comment: str, args) -> None:
    issue_number = _resolve_issue_number(args.issue_number)
    if issue_number is None:
        return
    repo = _detect_repo(args.github_repo)
    ...
    if response.status_code >= 300:
        raise SystemExit(f"Failed to publish GitHub comment ({response.status_code}): {response.text}")
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Publish threshold summaries to GitHub issues for long-term tracking.")
    ...
    parser.add_argument("--github-repo", help="GitHub repository in org/repo form.")
    parser.add_argument("--issue-number", type=int, help="Issue number that receives the comment.")
    parser.add_argument("--token", help="GitHub token (defaults to env).")
    parser.add_argument("--api-base", default="https://api.github.com", help="Override GitHub API base URL.")
    ...
    if issue_number is not None:
        _post_github_comment(comment, args)
        targets += 1
