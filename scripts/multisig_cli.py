...
def save_multisig_config(multisig_wallet):
    ...
    with open(os.path.join(WALLET_DATA_DIR, f"multisig_{multisig_wallet.multisig_address}.json"), "w") as f:
        json.dump(config, f, indent=4)
    print(f"Multisig wallet config saved to {WALLET_DATA_DIR}/multisig_{multisig_wallet.multisig_address}.json")
...
def main():
    ...
    # Generate Key Pair
    generate_parser = subparsers.add_parser("generate-key", help="Generate a new private/public key pair.")
    generate_parser.add_argument("--name", required=True, help="A name to identify this key pair (e.g., owner1).")

    # Create Multisig Wallet
    create_multisig_parser = subparsers.add_parser("create-multisig", help="Create a new multi-signature wallet.")
    create_multisig_parser.add_argument("--required", type=int, required=True, help="Number of signatures required (m).")
    create_multisig_parser.add_argument("--public-keys", nargs='+', required=True, help="List of public keys (hex strings) of the owners (n).")

    # Create Transaction
    create_tx_parser = subparsers.add_parser("create-tx", help="Create a new transaction for a multisig wallet.")
    create_tx_parser.add_argument("--multisig-address", required=True, help="Address of the multisig wallet.")
    ...
    create_tx_parser.add_argument("--output-file", default="unsigned_tx.json", help="Filename to save the unsigned transaction.")

    # Sign Transaction
    sign_tx_parser = subparsers.add_parser("sign-tx", help="Sign an existing transaction with a private key.")
    sign_tx_parser.add_argument("--private-key-name", required=True, help="Name of the private key to use for signing.")
    sign_tx_parser.add_argument("--transaction-file", required=True, help="Path to the unsigned transaction JSON file.")
    sign_tx_parser.add_argument("--output-file", help="Optional: Filename to save the signed transaction (defaults to overwriting input).")

    # Verify Transaction
    verify_tx_parser = subparsers.add_parser("verify-tx", help="Verify if a transaction has enough valid signatures.")
    verify_tx_parser.add_argument("--multisig-address", required=True, help="Address of the multisig wallet.")
    verify_tx_parser.add_argument("--transaction-file", required=True, help="Path to the signed transaction JSON file.")
    ...
    if args.command == "generate-key":
        ...
        for sig, pk in transaction["signatures"]:
            if pk == pub_key_hex:
                print(f"Warning: Key '{args.private_key_name}' has already signed this transaction.")
        ...
        print(f"Transaction signed by {args.private_key_name}. Current signatures: {len(transaction['signatures'])}")
