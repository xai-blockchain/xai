...
from scripts.node_profile import (
    ...
    EMERGENCY_ADMIN_PUBLIC_KEY_HEX,
)
...
def main():
    ...
    parser.add_argument(
        "--node-data-dir",
        required=True,
        help="Path to the node's data directory (e.g., ./data/node01).",
    )
    parser.add_argument(
        "--admin-key-name",
        default="admin_owner",
        help="Name of the admin key pair to use for signing the config update.",
    )
    parser.add_argument(
        "--use-emergency-key", action="store_true", help="Use an emergency admin key for signing."
    )
    parser.add_argument(
        "--enactment-delay-hours",
        type=int,
        default=0,
        help="Time-lock delay for config changes in hours (0 for immediate).",
    )
    parser.add_argument(
        "--new-key-name",
        required=True,
        help="Name to save the new validator key pair (e.g., validator_key_new).",
    )
    ...
    if not config_path.exists():
        print(
            f"Error: Node config file not found at {config_path}. Please run node_wizard.py first."
        )
        return
    ...
    updated_node_payload["miner"] = (
        new_pub_key  # Assuming miner address is the validator's public key
    )
    ...
    if not signer_priv_key or not signer_pub_key:
        print(
            f"Admin key pair '{args.admin_key_name}' not found. Please generate it using 'python scripts/multisig_cli.py generate-key --name {args.admin_key_name}' first."
        )
        return
    ...
    if args.use_emergency_key:
        if signer_pub_key != EMERGENCY_ADMIN_PUBLIC_KEY_HEX:
            print(
                f"Error: Public key for '{args.admin_key_name}' does not match the designated EMERGENCY_ADMIN_PUBLIC_KEY_HEX."
            )
            return
    else:
        if signer_pub_key != ADMIN_PUBLIC_KEY_HEX:
            print(
                f"Error: Public key for '{args.admin_key_name}' does not match the designated ADMIN_PUBLIC_KEY_HEX."
            )
            return
    ...
    if args.enactment_delay_hours > 0:
        ...
        print(
            f"Config change will be time-locked and enacted at: {datetime.fromtimestamp(enactment_timestamp, timezone.utc)} UTC"
        )
