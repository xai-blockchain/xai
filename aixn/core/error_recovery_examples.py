...
# Example 1: Basic Integration with Blockchain
...
def example_api_usage():
    ...
    # Get health metrics
    response = requests.get(f"{base_url}/recovery/health")
    print(f"Health: {response.json()}")

    # Get circuit breaker states
    response = requests.get(f"{base_url}/recovery/circuit-breakers")
    ...
    # List backups
    response = requests.get(f"{base_url}/recovery/backups")
    print(f"Available Backups: {response.json()}")

    # Check for corruption
    response = requests.post(f"{base_url}/recovery/corruption/check")
    print(f"Corruption Check: {response.json()}")

    # Get error log
    response = requests.get(f"{base_url}/recovery/errors?limit=10")
