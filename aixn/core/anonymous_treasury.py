...
class AnonymousTreasury:
    ...
    def __init__(self, data_dir=None):
        ...
        self.treasury_address = "AIXNTREASURY_ANONYMOUS_DEVELOPMENT_FUND"
    ...
    def propose_spending(self,
                        category: SpendingCategory,
                        amount: float,
                        purpose: str,
                        recipient_type: str = "anonymous") -> dict:
        """
    ...
    def execute_spending(self, tx_id: str) -> dict:
        ...
        if tx['status'] != 'pending':
            return {
                'success': False,
                'error': f'Transaction already {tx["status"]}'
            }
        ...
        if category not in self.stats['spending_by_category']:
            self.stats['spending_by_category'][category] = {
                'count': 0,
                'total_spent': 0.0
            }
    ...
    def get_balance(self) -> dict:
        ...
        return {
            ...
            'last_updated_date_utc': datetime.fromtimestamp(
                self.balance_data['last_updated_utc'],
                tz=timezone.utc
            ).strftime('%Y-%m-%d %H:%M:%S UTC')
        }
