...
class WalletClaimSystem:
    ...
    def _load_wallets(self):
        ...
        print(f"  Premium wallets available: {len([w for w in self.premium_wallets if not w['claimed']])}")
        print(f"  Standard wallets available: {len([w for w in self.standard_wallets if not w['claimed']])}")
        print(f"  Micro wallets available: {len([w for w in self.micro_wallets if not w.get('claimed', False)])}")
    ...
    def _finalize_claim(self, node_id: str, wallet: dict, tier: str, proof: str = None) -> dict:
        ...
        response = {
            ...
            f'remaining_{tier}': len([w for w in getattr(self, f"{tier}_wallets") if not w.get('claimed')])
        }
        return response
    ...
    def claim_premium_wallet(self, node_id: str, proof_of_mining: str = None) -> dict:
        ...
        existing_claim = next((c for c in self.claims['premium_claims'] if c['node_id'] == node_id), None)
        if existing_claim:
            wallet = next((w for w in self.premium_wallets if w['address'] == existing_claim['address']), None)
            return {'success': True, 'tier': 'premium', 'wallet': wallet, 'message': 'Premium wallet already claimed.'}
    ...
    def claim_standard_wallet(self, node_id: str) -> dict:
        ...
        existing_claim = next((c for c in self.claims['standard_claims'] if c['node_id'] == node_id), None)
        if existing_claim:
            wallet = next((w for w in self.standard_wallets if w['address'] == existing_claim['address']), None)
            return {'success': True, 'tier': 'standard', 'wallet': wallet, 'message': 'Standard wallet already claimed.'}
    ...
    def claim_bonus_wallet(self, miner_id: str, proof_of_mining: Optional[str] = None) -> dict:
        ...
        existing_claim = next((c for c in self.claims['bonus_claims'] if c['node_id'] == miner_id), None)
        if existing_claim:
            wallet = next((w for w in self.bonus_wallets if w['address'] == existing_claim['address']), None)
            return {'success': True, 'tier': 'bonus', 'wallet': wallet, 'message': 'Bonus wallet already claimed.'}
    ...
    def claim_micro_wallet(self, node_id: str) -> dict:
        ...
        existing_claim = next((c for c in self.claims.get('micro_claims', []) if c['node_id'] == node_id), None)
        if existing_claim:
            wallet = next((w for w in self.micro_wallets if w['address'] == existing_claim['address']), None)
            return {'success': True, 'tier': 'micro', 'wallet': wallet, 'message': 'Micro wallet already claimed.'}
        ...
        if tier == 'premium':
            ...
            for wallet in all_premium:
                if wallet['tier'] == 'miner':
                    updated = next((w for w in self.premium_wallets if w['address'] == wallet['address']), None)
                    if updated:
    ...
    def get_node_wallet(self, node_id: str) -> dict:
        ...
        premium_claim = next((c for c in self.claims['premium_claims'] if c['node_id'] == node_id), None)
        if premium_claim:
            wallet = next((w for w in self.premium_wallets if w['address'] == premium_claim['address']), None)
            return {'tier': 'premium', 'wallet': wallet}

        standard_claim = next((c for c in self.claims['standard_claims'] if c['node_id'] == node_id), None)
        if standard_claim:
            wallet = next((w for w in self.standard_wallets if w['address'] == standard_claim['address']), None)
    ...
    def get_stats(self) -> dict:
        ...
        total_premium_value = sum(w['total_balance'] for w in self.premium_wallets if not w['claimed'])
        total_standard_value = sum(w['initial_balance'] for w in self.standard_wallets if not w['claimed'])
