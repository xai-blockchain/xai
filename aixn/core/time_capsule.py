...
class TimeCapsuleManager:
    ...
    def capsule_address(self, capsule_id: str) -> str:
        _, vk = self._capsule_keypair(capsule_id)
    ...
    def register_lock_transaction(self, tx: "Transaction", block_height: int, block_timestamp: float):
        ...
        self._save_capsules()
        self._log_event('time_capsule_buried', {
            'capsule_id': capsule_id,
    ...
    def build_claim_transaction(self, capsule: TimeCapsule) -> Optional["Transaction"]:
        ...
        sk, _ = self._capsule_keypair(capsule.capsule_id)
        claim_tx.sign_transaction(sk.to_string().hex())
    ...
    def register_claim_transaction(self, tx: "Transaction", block_timestamp: float):
        ...
        self._save_capsules()
        self._log_event('time_capsule_opened', {
            'capsule_id': capsule_id,
            'txid': tx.txid,
            'amount': capsule.amount,
    ...
    def _value_snapshot(self, capsule: TimeCapsule) -> Dict[str, Any]:
        address = capsule.metadata.get('capsule_address') or self.capsule_address(capsule.capsule_id)
        ...
        if delta_since_last > 0:
            message = (
                f'Time Capsule value whispered up by {delta_since_last:.2f} XAI since the last check-in—still safe and secure.'
            )
    ...
    def create_xai_capsule(self, creator: str, beneficiary: str, amount: float, unlock_time: int, message: str = "") -> Dict[str, Any]:
        if amount <= 0:
    ...
    def create_cross_chain_capsule(self, creator: str, beneficiary: str, coin_type: str, amount: float, unlock_time: int, htlc_hash: str, origin_chain_tx: str, message: str = "") -> Dict[str, Any]:
        if unlock_time <= int(time.time()):
            return {'success': False, 'error': 'Unlock time must be in the future'}
    ...
    def claim_capsule(self, capsule_id: str, claimer: str, htlc_preimage: Optional[str] = None) -> Dict[str, Any]:
        capsule = self.capsules.get(capsule_id)
        if not capsule:
            return {'success': False, 'error': 'Capsule not found'}
        if not capsule.is_unlocked():
            return {'success': False, 'error': f'Capsule locked for {capsule.days_remaining():.1f} more days'}
        if capsule.claimed:
        ...
        self._save_capsules()
        return {'success': True, 'message': 'Time Capsule ready to be unearthed via the automated claim transaction'}
