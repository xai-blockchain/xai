...
class ValidationReport:
    ...
    def add_issue(self, severity: str, block_index: Optional[int], issue_type: str,
...
class ChainValidator:
    ...
    def validate_chain(self, blockchain_data: dict,
                      expected_genesis_hash: Optional[str] = None) -> ValidationReport:
        """
        ...
        if not chain or len(chain) == 0:
            self.report.add_issue(
                'critical', None, 'empty_chain',
                'Blockchain is empty - no blocks found'
            )
        ...
        self.report.genesis_valid = self._validate_genesis_block(
            chain[0], expected_genesis_hash
        )
        ...
        self.report.success = (
            self.report.genesis_valid and
            self.report.chain_integrity and
            self.report.pow_valid and
            self.report.signatures_valid and
            self.report.balances_consistent and
            self.report.supply_cap_valid and
            self.report.merkle_roots_valid and
            len(self.report.get_critical_issues()) == 0
        )
    ...
    def _validate_genesis_block(self, genesis_block: dict,
        ...
        if genesis_block.get('previous_hash') != "0":
            self.report.add_issue(
                'critical', 0, 'genesis_previous_hash',
        ...
        if expected_hash and genesis_block.get('hash') != expected_hash:
            self.report.add_issue(
                'critical', 0, 'genesis_hash_mismatch',
        ...
        if genesis_block.get('hash') != calculated_hash:
            self.report.add_issue(
                'critical', 0, 'genesis_hash_invalid',
    ...
    def _validate_transaction_signatures(self, chain: List[dict]) -> bool:
        ...
        for i, block in enumerate(chain):
            ...
            # Progress indicator
            if self.verbose and i % 1000 == 0 and i > 0:
                print(f"  Validated signatures for {tx_count} transactions in {i}/{len(chain)} blocks...")
    ...
    def _rebuild_utxo_set(self, chain: List[dict]) -> Tuple[Dict[str, List[dict]], float]:
        ...
        for i, block in enumerate(chain):
            ...
            # Progress indicator
            if self.verbose and i % 1000 == 0 and i > 0:
                print(f"  Rebuilt UTXO set for {i}/{len(chain)} blocks (supply: {total_supply:,.2f} XAI)...")

        if self.verbose:
            print(f"  ✓ UTXO set rebuilt: {len(utxo_set)} addresses, {total_supply:,.2f} XAI total supply")

        return utxo_set, total_supply

    def _validate_balance_consistency(self, chain: List[dict],
                                     utxo_set: Dict[str, List[dict]]) -> bool:
        """
        ...
        for address, utxos in utxo_set.items():
            ...
            if balance < 0:
                self.report.add_issue(
                    'critical', None, 'negative_balance',
    ...
    def _validate_supply_cap(self, total_supply: float) -> bool:
        ...
        if total_supply > self.max_supply:
            self.report.add_issue(
                'critical', None, 'supply_cap_exceeded',
        ...
        if self.verbose:
            if valid:
                print(f"  ✓ Supply cap verified: {total_supply:,.2f} / {self.max_supply:,.0f} XAI ({(total_supply/self.max_supply)*100:.2f}%)")
            else:
        ...
        for i, block in enumerate(chain):
            ...
            if stored_merkle_root != calculated_merkle_root:
                self.report.add_issue(
                    ...
                    {
                        'expected': calculated_merkle_root,
                        'actual': stored_merkle_root
                    }
                )
                valid = False
...
def validate_blockchain_on_startup(blockchain_data: dict,
                                   max_supply: float = 121000000.0,
                                   expected_genesis_hash: Optional[str] = None,
                                   verbose: bool = True) -> Tuple[bool, ValidationReport]:
    """
